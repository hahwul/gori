...
- MCP: a "Tell the agent…" message now reaches **every** attached client, not just the two with a live route — whatever the operator said rides back on the next gori tool the agent calls, as a second content block beside that tool's own answer. grok, Pi, Hermes, Antigravity and Claude Desktop expose no way for a server to knock on a running session, so until now they depended on the model remembering to call `operator_messages`; the handshake instructions that ask it to are delivered once, at initialize. The carry is confirmed, so it retires the message like the inbox socket does, and a read-only server still sends the line. The notification ring says `→ … got it (on its next tool result)`, and the palette verb's empty state no longer names Claude Code as the only client gori installs into (#1090)
