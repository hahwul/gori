...
- Empty states: every tab that can be empty now draws a small figure above its card, and the four that never had a card get one. The figures are a new `EmptyArt` catalog — one per variant, three rows (four or five for the two editor panes, which spend no row on a headline), drawn in ASCII, Box Drawing and Block Elements only because the draw places glyph N at column N and a width-2 glyph shears the row. Size is the whole design: a body is 14–16 rows on an 80×24 and a card eats 6–15 of them, which is why the one figure that already existed — the gori rings on the Project tab, at 11 rows needing ~20 — was gated out of every realistic pane and read as absent rather than as opportunistic. The Project tab now has a mark of its own, a dossier rather than the product logo, and the shared `Brand::ART` is left to the project picker and Help → About. Discover, Comparer, Miner and Sequencer showed one grey line where their siblings showed a card — sharpest inside `Target`, where Sitemap drew the full card and Discover drew a sentence; all four now reach the same module Repeater and Fuzzer already did. Both new whole-rect branches close a render/hit-test gap while they are at it: Discover declines `pane_at` with no runs, and Comparer's REQ⇄RES chips stop being clickable on a row the card is drawing, each from the one predicate its renderer reads. Colour is three depths of one hue rather than three palette slots — `Theme.border` was invisible on the default dark theme and `Theme.muted` outshone the mass it frames on `goriday`, so the tiers are now derived and ordered by construction in every theme

- Empty states, measured: the figures shipped, then the panes they ride in were measured on the terminals people actually run, and three cards turned out to be decided by a single row. Sitemap's wanted 12 against the 11 an 80×24 leaves its tree, Probe's 11 against 10 — both fell through to the three-line medium form, which inside `Target` put a full card and a bare sentence one chip apart, since Discover's pane pays for no filter bar or column header. Each gives back the one blank that bought nothing (the gap between the address and the hint it belongs with) and a spec now pins both at the measured heights. The Project dossier had the same problem one size up: five rows needed 14 and DESCRIPTION gets 13 on a 100×30, so the figure added *because* `Brand::ART` never fit was itself gated out of everything short of 120×40. It is four rows now, and the CENTERED budget is capped at four rather than five so it cannot grow back. OAST was the last workbench tab with no card at all — and the emptiest screen in the app — so it gets one: a listener handing out a payload and the callback returning on a channel it never sent on, leading with the Providers sub-tab when nothing is configured, because `g` refuses without a provider and "get a payload URL" would be a dead end as step one. The Miner's FINDINGS and the Sequencer's SAMPLES answered "no run yet" with one grey line where the Fuzzer's RESULTS drew a card for the identical moment; all three share one compact shape now, and none of them wears its container's name any more — a card titled RESULTS inside a pane titled RESULTS reads as a rendering fault. The states that REPORT on a run keep their own lines: "none found in the N of M names the budget allowed" is the one claim that pane must not lose to a generic card, and a filtered miss needs the query back, not an explanation of the tab. The Notes figure was redrawn as a page hanging off a tab strip, active chip lit — what the card under it already says the tab is. Its torn-off sheet had two problems and only one was the glyph: `╲`/`╱` are full-cell diagonals, so the tear came out as the loudest mass in a figure whose subject was the writing, and a corner promises the straight edge a sawtooth then refuses, so it read as a broken box rather than a torn one. The deeper problem was that a lone framed page is the same OUTLINE as the Project dossier — two neighbouring tabs differing only in which row was lit. Also: the Fuzzer's column header stops drawing over an empty list, DIST stops repeating the message the pane beside it is already showing, and the Miner figure's two sieve survivors now sit on the same sub-grid instead of one of them straddling a divider

- Host overrides: an override on gori's own reserved name now works from either layer. `reserved_self_host?` — the escape hatch that hands a real LAN box called `gori` back to its owner instead of answering with the setup page — read the per-project table alone, while the dial it mirrors reads project then global, so an operator who wrote the override in `settings.json` got the setup page or a 502 naming the reserved host, with no way out. Both now ask one method. The same predicate matched the host byte-for-byte where `magic_host?` chomps a trailing root dot, which was one instance of a wider mismatch: both layers looked up by a lowercased host and nothing else, so `Host: example.com.` never matched an entry for `example.com` while `example.com.` was storable as an entry no request could ever match. One key form now (case and a trailing root dot folded) on both sides of both layers, applied to rows written before it too, and `HOST_RE` refuses the leading dot a cookie-domain paste leaves behind. Separately, the mutex `connect_address` takes on every proxied request was being held across the store round-trip, so a save from a second gori process could stall every dial in a project with any override for as long as it held the SQLite write lock; the writers have their own lock now
