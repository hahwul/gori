...
- Session slots: a slot can re-authenticate itself by replaying a short list of Repeater sessions (for example CSRF fetch, then login), by hand (`Ctrl-R` in the slot picker, `gori run session refresh`, MCP `refresh_session_slot`) or on its own before a send when its `jwt-exp` / `ttl=` policy says the token is about to expire. Steps are added with Repeater `Space` → `b`, `session edit --refresh` or MCP `refresh`, are recorded in History as `src:refresh`, and never retry a request after a 401 (#1233)
