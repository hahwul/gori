...
- TUI: the view toggles (hex, pretty, diff, folds, static assets, follow, columns, Probe's show closed, the Params tab's all headers…) and the Repeater/Fuzzer transport toggles (HTTP/2, SNI, auto Content-Length, gRPC, TLS fingerprint…) move into two sticky space-menu cards, `Z` **Display…** and `P` **Protocol…**, that show each toggle's state; hex is `Z x` on every pane. The detail's Copy flow row folds into Copy as… → Raw request, and direct keys are unchanged (#1274, #1295)
