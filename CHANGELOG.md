...
- Project settings: a host override reaches gori's own reserved name from either layer and folds case and a trailing root dot into one key; the TUI refreshes the project env table before writing it back over a peer's edit; and the Project tab's overview folds its rows instead of dropping the ones it had no room for (#687, #689, #691)
...
- MCP: `gori mcp --install-claude` now writes Claude Desktop's config where the running platform actually keeps it — `$XDG_CONFIG_HOME/Claude/` (default `~/.config/Claude/`) on Linux, not a macOS `~/Library/Application Support/…` path built under a Linux `$HOME`. The file landed, gori reported success with a real path, and the client that never reads that path showed no gori tools at all. The other four install targets were already the same on every platform (#718)
