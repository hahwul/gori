...
- Env: `$GEN.USER_AGENT` fills in a real desktop browser User-Agent, drawn at random per send from a list built into gori, and `$GEN.USER_AGENT_CHROME`/`_FIREFOX`/`_SAFARI` narrow it to the browser a TLS preset of that name claims; `$U` + ↹ now completes to it rather than `$GEN.UUID`. Your own list replaces the built-in one from Settings → User-Agents or `gori settings user-agents` (#1112, #1152, #1154)
