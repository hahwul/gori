...
- Issues: `gori run issues create` and `update` accept notes inline, from a file, or from stdin without exposing long security notes in the argument list; the input modes are mutually exclusive, multiline UTF-8 and line endings are preserved, and create stores the notes in its initial transaction (#1017)
