...
- MCP: `create_issue` takes `notes`, so a finding is filed with its write-up in one call. It was the only creation tool on the surface that could not write its own body — `create_note` has `text` and `update_issue` has `notes` — so an agent filing findings in bulk had to follow every create with an `update_issue(notes:)`: two writes per issue, non-atomic, and an issue that existed bodiless in between. The body is written by the insert itself, the same transaction `gori run issues create --notes/--notes-file/--notes-stdin` already advertises, and it is masked like the title and host beside it. Omitting it still creates a bodiless issue. (#1076)
