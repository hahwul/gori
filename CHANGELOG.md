...
- MCP: a "Tell the agent…" message to a **Codex** session lands in that session's own thread instead of waiting to be polled — `gori mcp` hands it over with `codex queue --thread`, and the session runs a turn on it (right away when idle). The thread and its `CODEX_HOME` are read from the writer lock the parent Codex process holds open, so a session started with its own home is still reached; a thread that has not run a turn yet says so on the delivery row and stays readable through `operator_messages` (#1090)
- MCP: **Messages to the agent** — from the command palette ("Tell the agent…") or the Attached-agents card (`t`), send a one-line message to an attached agent's own session, with any marked flows as context; it is delivered by whichever of three routes is available — a research-preview `claude/channel` push (the **AI → Agent messaging** setting, off by default), the Claude Code inbox socket, or the `operator_messages` poll tool every other agent has — and each attempt is reported in the notification ring. The agent answers with `reply_to_operator`: a one-line summary in the ring and in Miss Ring's bubble (up to three lines), the full detail behind `↵` in a scrollable card. Clients with no live route of their own — grok, Pi, Hermes, Antigravity, Claude Desktop — get the message on the result of the next gori tool the agent calls, beside that tool's own answer (#1090)
