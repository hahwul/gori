...
- Project: the `description` a project is created with is readable again outside the TUI — `gori run project list --format json` and MCP `project_info` both report it (#1108)
- CLI: `gori run project delete`'s preview reports both locks the delete honours, so it no longer invites `--yes` for a project another gori instance has open or whose capture lock it could not read (#1108)
...
- MCP: three argument refusals name what the caller actually sent — an omitted `import_flows{kind}` is reported as missing rather than invalid, a `set_headers` entry that is a `{"Cookie": "…"}` map is refused by its shape instead of as the empty `": "` it was folded into, and `send_request{headers}` now takes the `[{name, value}]` list its sibling tools take (#1105)
- CLI: `gori run grpc reflect --timeout` and the Fuzzer, Discover, Miner and Sequencer `--rate` reject non-finite numeric values; an out-of-range reflection timeout is reported as a usage error instead of a Crystal backtrace, and shared rate errors no longer misname every command as `gori run fuzz` (#1104)
- MCP: cancelling a request now STOPS the work instead of only suppressing its answer — an `active` `probe_scan`, a `minimize_repeater` search and a `run_retest` stop sending at the next flow, candidate or step, so a client that has stopped waiting no longer has hundreds of real requests sent at the target on its behalf; a cancelled minimize also leaves the stored request alone (#1103)
- MCP: a server no longer promises the agent tools it does not have — the handshake instructions named ten absent tools under `--tools='list_*'`, starting with the very first one they told it to call, and went on offering `delete_project` to every `--read-only` server — and every start now reports how many tools it is about to load into the model's context, the count it will actually advertise rather than the whole registry, not only the starts that already passed `--tools` (#1102, #1105)
- MCP: `gori mcp` speaks the stateless **`2026-07-28`** revision alongside the handshake ones — `server/discover`, per-request `_meta` version negotiation, `resultType` and server identity on every result, `tools/list` cache hints, `subscriptions/listen`, and `-32022` with the list of versions it does speak for one it does not; `2025-11-25` joins the handshake set. Every tool now advertises `annotations.readOnlyHint`, so a client can tell the tools that only read the capture from the ones that write to it or send traffic, an unknown tool name comes back as a protocol error rather than a tool result, and the `claude/channel` research preview is offered only to handshake-era clients — offered, and not withdrawn again by the next `server/discover` probe, while a liveness `ping` is answered even when the worker happens to be mid-batch (#1100, #1105)
...
- Discover: a `-H` header name that is not an RFC 7230 token — a non-ASCII name, or one carrying `/`, `(`, `,` — is refused and reported rather than written onto the wire; the field-name predicate now has one home beside the request-token one (#1086)
...
- Probe: four rules stop guessing — PHP's browser-visible `Warning`/`Notice` output is read as an error disclosure, the CORS origin-reflection probe runs on any endpoint advertising `Vary: Origin`, the NGINX alias probe tests a two-segment `location` too, and DOM clobbering no longer fires on the `window.X = window.X || {}` preamble every bundle ships (#1098)
...
- MCP: **Messages to the agent** — from the command palette ("Tell the agent…") or the Attached-agents card (`t`), send a one-line message to an attached agent's own session, with any marked flows as context. gori tries the routes that can confirm themselves first — the Claude Code inbox socket, then `codex queue --thread` into a live Codex session's own thread (read from the writer lock that process holds open, so a session with its own `CODEX_HOME` is still reached) — then the research-preview `claude/channel` push (the **AI → Agent messaging** setting, off by default); a dead socket falls through instead of ending the attempt, and every outcome is reported in the notification ring. Anything no route carried rides back on the result of the next gori tool the agent calls, beside that tool's own answer, so grok, Pi, Hermes, Antigravity and Claude Desktop are reached too, and `operator_messages` stays the backstop under all of it. The agent answers with `reply_to_operator`: a one-line summary in the ring and in Miss Ring's bubble (up to three lines), the full detail behind `↵` in a scrollable card. The send card refuses instead of claiming: an agent that detached while you were typing, and a write the project was too busy to take, are both said rather than toasted as sent — neither could ever have reached the ring — and a long client name no longer paints over the target picker's border (#1090)
