...
- Cache: web cache testing helpers. A `cache:` QL field (`hit`/`miss`/`dynamic`/`none`) reads the response cache headers on every surface and adds a CACHE column to History; `cache_deception_check` (MCP) and `gori run cache-deception` prime a flow as authenticated then re-request it anonymously to catch a private response served from a cache; and a `cache-delimiters` Fuzzer payload set sweeps the path with cache-key delimiters (#1247)
...
- Rewriter: unknown rule labels stay visible and inert, and survive settings saves (#1242)
