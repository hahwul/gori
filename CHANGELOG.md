...
- Projects: export and import compact, WAL-safe `.gori` snapshots from the CLI, the picker and MCP `export_project` / `import_project` with a sensitive-data inventory; archives stay unredacted, while imports disable executable/file-backed rules, reset project routing, and reject archives over 2 GiB uncompressed, and an MCP import only previews until `confirm:true` (#1230, #1250, #1253, #1271)
