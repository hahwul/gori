...
- Retest: a diff row now reaches Issues and Notes. `⇧F` files the selected endpoint as an Issue prefilled with what the retest observed, `n` records it as a Note in one keystroke, and neither moves you off the tab. Both carry the context only Diff holds — which two projects were compared and where their databases are, what each side answered, which axis moved — and link the capture on whichever side is the open project as evidence rather than copying it (`entity_links` do not cross projects, so the other side's flow is named with its database). A coverage-gap row files as `info` and says the newer capture never requested the endpoint; it never claims a removal the diff did not observe, and `gori run diff --format json` now carries that same sentence on every row (#845)
...
- External process hooks: pipe bytes through your own command instead of a plugin API — a Rewriter `pipe` op, a Decoder chain `exec:` step, and a Probe `exec` rule, all one primitive. The command is exec'd directly with no shell and runs with YOUR privileges (unsandboxed, the same trust level as any Rewriter rule); every run has a hard timeout (`hooks.timeout_secs`, default 5s) and a 32 MiB stdout cap, and on timeout, non-zero exit, failed spawn or oversized output the original bytes pass through unchanged with a notice — the proxy never stalls on a hook. The MCP `decode` tool refuses `exec:` steps: it stays pure compute (#818)
...
- TUI: syntax-highlight HTTP structure and structured bodies in the History Request/Response preview (#834)
