...
- TUI: the project picker finds a project by its directory slug, short id or bound workspace path, not only by display name, and two projects sharing a display name carry what tells them apart on the row and in the delete confirm. A create or a rename it refuses now says why instead of leaving the form unchanged (#1107)
