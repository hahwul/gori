...
- TUI: Help, hint strips, toasts and the Repeater's border chips (`␣Pr:FRAME`, `␣Pw:KEY`, `␣Pt`) take space-menu letters and keys from the menu itself, fixing the rows that named the wrong key (Repeater Tag sub-tab, gRPC reframe, OAST add issue, Sitemap `tag:` help, the Rewriter's enable/disable, the Fuzzer's sort) (#1274, #1295)
