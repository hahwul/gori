...
- Settings profiles: a shareable profile can carry a command, and both ends now say so. `rewriter` rules with `op: pipe`, `scan_rules` with `kind: exec` and a named `decoder` chain step still export like any other rule — a team standardising on one re-signing hook is what hooks are for — but `gori settings export` reports how many it wrote on stderr, and `gori settings import` lists each one with its name, kind and argv before writing anything, then **refuses until you pass `--allow-commands`** (no prompt, so a scripted import stays scriptable). `--dry-run` prints the same list; narrowing with `--sections` narrows it too, so an import that arms nothing asks for nothing. `gori settings sections` marks the three sections that can carry an argv (#842)
