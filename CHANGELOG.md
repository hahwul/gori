...
- MCP: `gori mcp` speaks the stateless **`2026-07-28`** revision alongside the handshake ones — `server/discover`, per-request `_meta` version negotiation, `resultType` and server identity on every result, `tools/list` cache hints, and `-32022` with the list of versions it does speak for one it does not; `2025-11-25` joins the handshake set. Every tool now also advertises `annotations.readOnlyHint`, so a client can tell the tools that only read the capture from the ones that write to it or send traffic (#1100)
