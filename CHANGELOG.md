...
- CLI: short-lived `gori run` project writes use a bounded SQLite lock wait and, when a live gori owns the writer slot, say so with the read-only workaround instead of naming a `--db` target's directory as a project, while `discover`, `fuzz`, `import`, `probe`, `retest run`, `oast listen` and `intercept` keep the standard wait for the run they hold the project open for; a successful repeater send stays successful when its response or History cannot be saved, so automatic retries do not duplicate it; `--format json` now carries `response_saved` / `history_saved` with the reason, and a response written to a session another gori deleted mid-send is reported as such instead of as saved. A `discover` batch the writer refused is no longer dropped in silence: the CLI reports the count and exits 1, and MCP `discover_status`/`discover_results` carry `unsaved_flows` (#1118)
