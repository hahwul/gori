...
- TUI: syntax-highlight HTTP structure and structured bodies in the History Request/Response preview, and decode compressed request and response bodies there and in Fuzzer result details — the preview now reads both halves through the same entity seam the History detail pane always has, and a flow that re-fetches every frame (pending, 101, h2) no longer re-inflates its body behind an unchanged pane (#834)
