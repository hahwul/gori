...
- Repeater: a `$GEN.RANDOM_HEX` typed into a tab opened from History is sent as a fresh value, not as those fifteen characters. The send seam resolves `$BIND.` and `$GEN.` at the socket, and it was switched off WHOLE for a tab seeded from a capture — the rule that keeps a captured OData `$filter` from being rewritten into a project value. The env-var pass one layer up had stopped doing that per BUFFER years ago and does it per NAME instead (the capture's names stay literal, the operator's resolve), and the two send-time namespaces could not follow it: a binding resolves at the socket, and a generator must mint there, once per send. So a seeded tab offered `$GEN.RANDOM_HEX` in its completer, printed its format under the caret, and put the token itself in the request line — and History showed the token back, correctly, because the row is always the bytes that went out. The tab now hands the send seam the names the CAPTURE arrived with, so those alone stay literal; a capture's `$$BIND.x` is still not unescaped, and every headless replay of a whole captured flow (`gori run repeater <flow-id>`, MCP's flow path, `--bind-from`) keeps sending the stored bytes untouched. The send toast's "sent literally" note follows the same line, so it names only what was actually withheld. (#1069)
- Robustness: seven ways ordinary data could end the process, all of them the same shape — a parser raising a class the code around it had not imagined. A HAR carrying one byte that is not valid UTF-8 (a browser writing a response body verbatim) reached the streaming walk as `InvalidByteSequenceError`, not the `JSON::ParseException` it was written around, and ran out through the CLI as a backtrace; an OpenAPI `servers[0].url` with an over-large or non-numeric port raised out of `URI.parse`; a self-referential YAML anchor in that same spec raised out of `to_json`, past the clause that only covered YAML; a `match_rules` row whose `target` or `part` had drifted took `rewriter list` down, where the two enum readers beside them had been tolerant all along. Two more were arithmetic: every `Fmt` cell ended in `Float64#to_i`, Crystal's CHECKED conversion into **Int32**, so a HAR declaring `"time": 8e15` made the History row raise on the render path — three failed frames trip the tick breaker and end the session — and `to_local` refused a far-future instant in any timezone east of UTC, which is persistent once the row is stored: the same row printed under `TZ=UTC` and died under `TZ=Asia/Seoul`, leaving `--format json` writing a truncated document. Each now reports what it met, the formatters say a value is unspellable in the cell instead of taking the frame down, and every surface that renders a stored timestamp goes through one `LocalTime` helper that falls back to UTC rather than raising — nine call sites, seven of them TUI render paths and one a QL `date:` filter that raised while filtering rather than drawing. Found by fuzzing the decoders, the render helpers and the importers against their own stated contracts.
- MCP: `create_issue` takes `notes`, so a finding is filed with its write-up in one call instead of a create followed by an `update_issue`, which left the issue bodiless in between. The body rides the insert, the transaction `gori run issues create --notes` already uses. (#1076)
- Repeater: a request whose head never terminates stops being invisible. `repeater create` took a head ending `\r\n\r` — one byte short, which is exactly what shell `$(…)` leaves behind, since it strips trailing newlines — reported `created successfully`, stored it verbatim, and every later send put the malformed message on the wire with the origin's bare `400` as the only feedback. The stored bytes render identically to a well-formed request in the TUI, in `repeater list` and in `get_repeater_context`, and a request WITH a body survives the same truncation (the terminator is followed by the body), so in a two-tab workflow the POST tab works and the GET tab beside it silently does not. Nothing is refused and nothing is repaired — a head that never ends is itself a desync primitive, and the Repeater is where non-standard HTTP belongs — so the malformation is now SAID: `repeater create` and `repeater send` write one sentence to STDERR, `repeater list` marks the row (`head_unterminated` under `--format json`), the TUI's send toast carries the same clause beside the status it reports, and MCP's `create_repeater`, `update_repeater`, `get_repeater_context` and `send_request` all return `head_unterminated` with that sentence attached. Two engines are exempt, because for them the missing line is not a wire fact at all: HTTP/2 re-encodes the head as an HPACK field list that never carried it, and `WsEngine` re-emits and re-terminates a framed handshake on every send — marking either would accuse gori of sending bytes it does not send. A `ws_http_only` session, whose bytes go through the HTTP engine untouched, is marked like any other. (#1075)
