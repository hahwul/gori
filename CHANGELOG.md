...
- TUI: **Agent tab** — hosts the operator's own Claude Code as a child process, with this project's MCP tools from its first turn, a permission card for the tool calls it would otherwise decide alone, per-project conversation history and `--resume` after the child exits, and a global "Ask the agent" quick prompt from any tab; it takes over Notes' slot on the nine-tab bar, and Notes moves behind `0` (#1093)
