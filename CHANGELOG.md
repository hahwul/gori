...
- Cache: web cache testing helpers. A `cache:` QL field (`hit`/`miss`/`dynamic`/`none`) reads response cache headers on every surface and shows CACHE in History for explicit `cache:` queries; `cache_deception_check` (MCP) and `gori run cache-deception` compare authenticated and anonymous responses with a cache-busted anonymous control to distinguish public content from a cached private response; and a `cache-delimiters` Fuzzer payload set sweeps the path with cache-key delimiters (#1247, #1256)
...
- Rewriter: switching the env token syntax no longer re-spells `$NAME` text inside a short-circuit stub's response, which gori sends as authored (#1237)
- Projects: export and import compact, WAL-safe `.gori` snapshots from the CLI and picker with a sensitive-data inventory; archives stay unredacted, while imports disable executable/file-backed rules, reset project routing, and reject archives over 2 GiB uncompressed (#1230, #1250, #1253)
