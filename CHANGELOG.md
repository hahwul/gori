...
- History: `gori run history --format json` redacts `Authorization`/`Cookie`/`Set-Cookie`/`Proxy-Authorization`/API-key header VALUES in each row's `headers` object, marking the row `sensitive_headers_redacted`. A listing is an inventory, and one run put live session material into terminal logs and agent transcripts; `--include-sensitive` returns the exact bytes, as it already did on `gori run intercept` (#1002)
