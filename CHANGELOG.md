...
- MCP: a server started with `--tools` no longer promises the agent tools it does not have — the handshake instructions named ten absent tools under `--tools='list_*'`, starting with the very first one they told it to call — and every start now reports how many of the 179 tools it is about to load into the model's context, not only the starts that already passed `--tools` (#1102)
