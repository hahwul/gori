...
- MCP: **Messages to the agent** — the palette's "Tell the agent…" sends a one-line message to an attached agent's own session, delivered by whichever of three routes is available: a research-preview `claude/channel` push (`mcp.channels` setting, off by default), the Claude Code inbox socket, or, for every other agent, the `operator_messages` poll tool; each attempt is recorded as an `agent_delivery` event and reported in the notification ring; the agent answers with `reply_to_operator` — a one-line summary in the ring and on Miss Ring, the detail behind ↵ (#1090)
