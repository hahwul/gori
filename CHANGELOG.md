...
- TUI: typing in the command palette (`Ctrl-P`) also finds the focused tab's actions, listed first under `THIS TAB` with their key and the marks count in the title, and the space menu drops 36 rarely used rows that the palette now finds: ones that repeat a direct key (Mark word `Ctrl-K`, rule reorder `⇧K`/`⇧J`) or configure once a session (Minimize request, Use as refresh for slot…). Their keys are unchanged, and Help shows `^P → <name>` for the ones with none (#1282)
...
- TUI: the view toggles (hex, pretty, diff, folds, static assets, follow, columns…) and the Repeater/Fuzzer transport toggles (HTTP/2, SNI, auto Content-Length, gRPC, TLS fingerprint…) move into two sticky space-menu cards, `Z` **Display…** and `P` **Protocol…**, that show each toggle's state; hex is `Z x` on every pane. The detail's Copy flow row folds into Copy as… → Raw request, and direct keys are unchanged (#1274)
...
- TUI: a bare key that belongs to one pane acts only there. The Repeater's `p` (pretty bodies) and `⇧D` (diff) work in the response pane, and the Fuzzer's `v` (distribution sidebar) in RESULTS, so they no longer fire in the request or template pane, whose space menus use those letters for other actions (#1274)
- TUI: Help, hint strips and toasts take space-menu letters from the menu itself, fixing the rows that named the wrong key (Repeater Tag sub-tab, gRPC reframe, OAST add issue, Sitemap `tag:` help); the guides now say `t` for a rule's enable/disable (#1274)
