...
`network` 섹션은 프록시가 어떻게 바인딩되는지, 그리고 트래픽을 업스트림 프록시로 전달할지에 대한 **전역 기본값**입니다. 자체 네트워크 재정의가 없는 프로젝트는 이 값을 상속합니다:
...
팔레트의 **Open browser** 동작은 이미 CA를 신뢰하고 프록시를 경유하는 격리된 프로파일로 설치된 브라우저를 실행합니다. 새 머신에서 가장 빠른 경로입니다([빠른 시작](/ko/getting-started/quick-start/) 참조).
