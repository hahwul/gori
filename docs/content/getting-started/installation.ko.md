...
설치 스크립트는 최신 릴리스가 무엇인지 GitHub API에 묻는데, 이 API는 **비인증 요청을 IP당 시간당 60회**만 허용합니다. 공용 CI나 NAT 뒤에서는 `403`이 돌아올 수 있습니다. 설치 스크립트와 `gori update` 모두 rate limit이 없는 릴리스 리다이렉트로 자동 폴백하므로 그대로 동작하며, `resolved v0.7.0 via ... (no API call)` 같은 줄이 보입니다.
...
버전이 붙은 이름(`gori-v0.7.0-linux-x86_64`)도 그대로 유지되니, 특정 빌드에 고정하려면 그쪽을 쓰세요.
...
`container build -f docker/Dockerfile -t gori:dev .`로 소스에서 이미지를 빌드할 수 있으며, BuildKit과 마찬가지로 `docker/Dockerfile.dockerignore`를 읽습니다. 빌더는 자체 VM에서 동작하므로 호스트의 `HTTP_PROXY`를 상속하지 않습니다.
