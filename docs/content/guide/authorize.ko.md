...
**refresh before** 정책은 슬롯으로 나가는 전송 직전에 갱신을 스스로 실행합니다. `jwt-exp`는 슬롯에 바인딩된 JWT의 `exp`가 30초 이내로 남았을 때, `ttl=10m`은 슬롯의 가장 최근 바인딩이 그보다 오래되었을 때입니다. 활성 슬롯뿐 아니라 Authorize 실행이 보내는 모든 아이덴티티에 적용됩니다. 정책은 전송 **전에** 동작하고 응답을 읽지 않습니다. 이 탭에서 `401`은 여전히 판정이며, 다시 로그인해 재시도할 이유가 되지 않습니다.
