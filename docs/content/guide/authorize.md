...
| `⇧X` | Clear — empty the queue and every identity's results (asks first). `Space` → `X` does the same; `⇧X` is the clear-all key in History, Probe and the Project ACTIVITY feed too |
