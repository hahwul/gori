...
A **refresh before** policy runs the refresh on its own, before a send that goes out as the slot: `jwt-exp` when a JWT bound in the slot is within 30 s of its `exp`, or `ttl=10m` when the slot's newest binding is older than that. It applies to every identity an Authorize run sends as, not only to the active slot. It acts **before** a send and never reads a response: a `401` in this tab is still the verdict, never a reason to log in again and retry.
