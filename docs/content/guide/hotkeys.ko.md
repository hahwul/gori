...
- **`d`는 파괴합니다.** 맨 `d`는 이 키를 바인딩한 모든 스코프(16곳)에서 선택한 행을 삭제하거나 무시합니다. Repeater의 응답 diff가 유일한 예외였고, 이제 `⇧D`로 옮겨서 반사적으로 누른 `d`가 표시 토글에 떨어지지 않습니다. 스페이스 메뉴에서는 `d` 글자를 그대로 두었습니다 — 메뉴를 읽는 것만으로 파괴되는 것은 없기 때문입니다. 삭제가 아닌 새 패널 동작은 `d`를 가져가지 않습니다.
...
- **하위 탭 스트립**이 어떤 동작에 이미 글자를 바인딩해 두었다면 메뉴도 같은 글자로 그 동작을 씁니다: `r`은 이름 변경, `f`는 하위 탭 목록·검색, `t`는 칩 마크, `⇧T`는 스트립 전체 마크입니다. 예외는 그 스코프의 COMMON 그룹이 훨씬 자주 쓰이는 동작에 이미 그 글자를 쓴 경우입니다 — Repeater·Fuzzer·Miner·Sequencer에서 `r`은 `Ctrl-R`을 메뉴에서 되울리는 Run/Send이므로, 이름 변경은 그 자리를 빼앗지 않고 `e`를 유지합니다.
