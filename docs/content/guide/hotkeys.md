...
- **`Ctrl-1`…`Ctrl-9` is undeliverable on many terminals** — there is no control character for it, so the sub-tab jumps simply never arrive. You never need it: on a sub-tab strip, press `←` from the first chip to reach the **`⌕`** affordance at the left edge, then `Enter` to list and search every open sub-tab (or just click it).
