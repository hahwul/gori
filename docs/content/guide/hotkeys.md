...
- **Shift** carries the whole-tab wipes. `⇧X` is `Clear` in every tab that has one — History, Probe, Authorize, and the Project ACTIVITY feed — with `X` as the space-menu letter beside it. The letter is `x` and not `c` because of what sits under the shift: bare `x` is bound in none of those four scopes, while bare `c` is live in all of them (`capture.toggle`, and `dismiss` on the Probe list), and a project wipe does not belong one shift above a key an operator presses all day. A destructive chord must also be **named where it can be read before it is pressed** — the Help sheet and the tab's own body hint, not the space menu alone — and it must ask first.
