...
반대 방향도 마찬가지입니다. `intercept_list`가 `edited: true`로 돌려주는 행은 지금 사용자가 편집 중인, 아직 저장하지 않은 내용이 들어 있는 메시지입니다. 에이전트는 그 메시지를 바이트 그대로 포워드해 작업을 지워버리는 대신 사용자에게 남겨둘 수 있습니다.
