...
| `screenshot` | 바인딩된 프로젝트 위에 TUI를 터미널 없이 그려 프레임을 파일로 저장(`svg` / `png` / `ansi` / `txt`). 목업이 아니라 실제 껍데기를 실제 저장소 위에 그리며, 읽기 전용으로 엽니다 — 포트도 캡처 잠금도 잡지 않고 아무것도 캡처하지 않으며 활성 프로젝트도 그대로라, 모든 샷에서 캡처 칩은 `off`로 보입니다. 디스크에 파일을 쓰므로 게이트되는 *쓰기* 도구지만 agent action은 아닙니다(프로젝트 안에서 바뀌는 것이 없습니다). 경로를 돌려주며, `inline:true`면 그림도 두 번째 content 블록으로 함께 옵니다(`png`는 이미지, 나머지는 문서 텍스트). 인코딩 기준 1 MiB를 넘으면 거절합니다. JSON 도구가 행은 주지만 모양은 주지 못할 때 패널의 **모습**을 보려면 이걸 쓰세요 |
