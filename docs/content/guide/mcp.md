...
| `operator_messages` | What the operator typed for you in the gori TUI ("Tell the agent…"), addressed to this session or to all attached agents, forward-cursored; gori delivers these live when it can (a channel event, or a peer note in Claude Code) and this is the fallback every agent has — call it at the start of a turn. Marks what it returns as delivered so the operator's ring can say "picked up" |
