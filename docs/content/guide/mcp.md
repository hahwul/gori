...
It reads the other way too. A row `intercept_list` returns with `edited: true` is one you have unsaved changes typed into right now, so an agent can leave that message to you instead of forwarding it byte-exact and discarding your work.
