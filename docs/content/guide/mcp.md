...
| `list_env` | Project env tokens available to `$ENV.KEY` substitution (values redacted). Each row also carries `length` and, when the value already begins with one, `scheme`, enough to tell whether a header should read `Bearer $ENV.KEY` or just `$ENV.KEY`, without the value. The result also reports the `syntax` and `prefix` in force, so an agent spells a token the way this install reads it |
...
| `set_env_var` / `delete_env_var` | Manage the project env tokens `$ENV.KEY` substitution reads. The key is stored bare; `list_env` reports which grammar spells it |
