...
| --- | ---: | ---: | ---: | --- |
| `gori mcp` | 185 | ~217 KB | ~56k | Everything (the default) |
...
| `cache_deception_check` | Test one flow for web cache deception: replay it as its captured (authenticated) identity, re-request the same url anonymously, then make a cache-busted anonymous control request. Matching control content is public (`served`); matching content with a cache hit and different control content is a likely deception (`cached`). Synchronous, up to three sends. Reads the response's `cache` status; pair it with the Fuzzer's `cache-delimiters` payload set to find the crafted paths that trigger it |
