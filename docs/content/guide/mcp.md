...
| `screenshot` | Draw the TUI headlessly over the bound project and write the frame to a file (`svg` / `png` / `ansi` / `txt`). The real chrome over the real store, opened view-only: no port bound, no capture lock, nothing captured, the active project unchanged — so the capture chip reads `off` in every shot. A gated *write* tool because it puts a file on disk, but not an agent action: nothing in the project changes. Returns the path; `inline:true` also returns the picture as a second content block (an image for `png`, the document as text otherwise), refused past 1 MiB encoded. Reach for it to **see** a pane's layout when the JSON tools give you rows but not the shape |
