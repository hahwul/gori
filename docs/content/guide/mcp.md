...
| `mine_start` / `mine_status` / `mine_results` / `mine_stop` | Drive the param miner. `names` are tested first, ahead of the built-in list and any wordlist (for example `list_params` names seen on the host's other endpoints) |
