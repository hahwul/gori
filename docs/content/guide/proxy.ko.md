...
`i`를 눌러 **Intercept**를 켭니다. 켜져 있으면 매칭되는 요청(그리고 선택적으로 응답)이 붙잡혀, 계속 진행하기 전에 포워드, 드롭, 편집할 수 있습니다. Intercept 탭 상단의 필터 바에서 붙잡을 방향을 고르고 쿼리 언어 표현식으로 붙잡을 대상을 좁힐 수 있어, 관심 있는 트래픽에서만 멈춥니다. `s` 스코프 렌즈가 켜져 있으면 조건과 관계없이 스코프 안의 트래픽만 붙잡습니다. 이 탭에서는 `c`가 붙잡을 방향을 순환하고(전체 / 요청만 / 응답만) `/`가 조건을 편집합니다. 캡처 토글 `c`는 다른 모든 탭에서 그대로입니다. 인터셉트를 **끄면** 아직 붙잡혀 있던 것이 전부 원래 바이트로 흘러가므로, 포워드하지 않은 편집은 버려집니다.
...
붙잡힌 메시지를 읽는 데는 표시도 편집기도 필요 없습니다. 미리보기는 줄을 접어 보여주므로 옆으로 벗어나는 내용이 없고, 휠로 스크롤하거나 `↵` / `e`로 편집기를 열면 `PgUp` / `PgDn`이 본문을 넘기고 `Ctrl-R`이 편집한 메시지를 포워드합니다(`Esc`는 편집을 유지한 채 편집기를 나갑니다). `PgUp` / `PgDn` / `Home` / `End`는 다른 목록 탭과 마찬가지로 **큐**를 넘깁니다. `↑` / `↓`가 걷는 그 목록입니다.
...
- **텍스트 메시지를 편집해도 줄 끝은 그대로입니다.** 페이로드는 각 줄의 원래 줄 끝을 유지한 채 나가며, 리피터와 HTTP 인터셉트 편집기도 마찬가지입니다. WebSocket 페이로드에서는 `$BIND.NAME` 바인딩이 확장되지 *않습니다*. 거기서 `$`는 참조가 아니라 바이트이기 때문입니다. 입력한 그대로 전송됩니다.
...
인터셉트는 연결을 강등하지 않고 HTTP/2에 적용되므로 캐치를 켠 채로도 gRPC 클라이언트가 계속 동작하며, 스트림을 **개별적으로** 붙잡습니다. 요청 하나를 붙잡아도 탭 전체가 멈추지 않습니다. HTTP/1.1과 다른 점이 넷 있습니다.

- **본문은 gori가 버퍼링할 수 있을 때, 그리고 그때만 붙잡습니다.** `Content-Length`가 1 MiB 이하로 선언된 메시지(그리고 본문이 아예 없는 메시지, 즉 대부분의 페이지 로드)는 통째로 붙잡힙니다. 헤드와 본문을 *모두* 보고 편집할 수 있고, 포워드하면 HTTP/1.1과 똑같이 입력한 바이트로 `DATA` 프레임을 다시 만들어 입력한 `Content-Length`로 보냅니다. 큐 행은 헤드가 도착한 시점이 아니라 메시지 전체가 도착한 뒤에 나타나며, 이 역시 HTTP/1.1과 같은 타이밍입니다.

  그 외의 모든 것은 **헤드만** 붙잡습니다. 길이가 선언되지 않은 스트리밍 업로드나 응답(SSE, gRPC 스트림), 상한을 넘는 본문, 패딩된 본문이 그렇습니다. 그런 본문은 게이트를 손대지 않고 그대로 지나가고, 편집기에 입력한 본문은 조용히 버려지는 대신 거부되며, `Content-Length`는 보낸 쪽이 설정한 값 그대로 유지됩니다. 인터셉트 편집기, `gori run intercept get`, MCP `intercept_get` 도구 모두 편집을 쓰기 전에 어떤 종류의 홀드인지 알려 주고, 헤드만 붙잡은 홀드의 본문도 이후 History에서 온전히 볼 수 있습니다.
...
- **붙잡힌 요청은 같은 연결의 이후 요청을 지연시킵니다.** HTTP/2는 새 스트림이 순서대로 원본에 도달할 것을 요구하므로, 붙잡힌 요청보다 *나중에* 시작된 요청은 결정을 기다립니다. gori가 그 요청의 본문을 아직 버퍼링하는 동안도 마찬가지입니다. 이미 진행 중인 요청은 계속 업로드되고, 응답은 모두 그대로 도착하며, 붙잡힌 *응답* 은 아무것도 지연시키지 않습니다.
- ***본문* Match & Replace는 여전히 연결을 HTTP/1.1로 내립니다.** 홀드가 이제 본문을 편집할 수 있게 되었어도 그렇습니다. 둘은 다른 거래입니다. 홀드는 이미 기다리고 있는 메시지 하나를, 눈에 보이는 길이 아래에서 버퍼링합니다. 반면 본문 규칙은 매칭되는 모든 메시지를 사람 손 없이 다시 써야 하고, 홀드가 버퍼링을 거절하는 스트리밍 메시지까지 포함해야 합니다.
...
| 동작 | 키 | 표시된 경로에 대해 |
|------|-----|------------------|
| 경로 태그 | `Space` `m` | 편집기 하나, 메모 하나를 표시된 모든 경로에 적용(비우면 전체 해제) |
| Repeater로 보내기 | `r` | 표시된 엔드포인트마다 서브탭 하나, 캡처된 플로우 기준으로 중복 제거(최대 20) |
| OpenAPI 내보내기 | `⇧E` | 표시된 모든 경로를 담은 문서 하나 |

따라서 `/ status:5xx` → `⇧T`로 필터가 보여주는 경로를 전부 표시(하나씩은 `t`) → `Space` → `m` → `auth`면 전체에 한 번에 태그가 붙고, 이후 `tag:auth`로 다시 찾을 수 있습니다. 메뉴 제목이 `SPACE · 3 MARKED`로 바뀌고 항목 이름도 `Tag 3 paths`, `Send 3 paths to Repeater`처럼 바뀝니다. Discover와 Sequencer는 단일 대상으로 남습니다(하위 트리 하나를 스캔하고, 엔드포인트 하나의 토큰을 수집하므로), 표시가 있는 동안 메뉴 항목에 `(cursor)`가 붙습니다.

여기서 **`t`는 표시, `⇧T`는 전체 표시, 태그는 `Space` → `m`**입니다. `t` / `⇧T`는 모든 목록 탭에서 표시 / 전체 표시라는 같은 뜻이므로, 이 탭에서 메뉴로 여는 쪽은 태그입니다. `⇧T`는 트리가 지금 보여주는 **캡처된 경로**를 전부 표시합니다 — 집합을 정하는 것은 `/` 필터와 펼쳐진 폴드입니다 — 그리고 호스트와 폴더 행은 건너뜁니다. 그렇지 않으면 그 아래 엔드포인트와 같은 묶음에 들어가 버리기 때문입니다. 합성 노드인 `{uuid}` / `[1, 2, 3 …]` 폴드도 실제 경로가 아니므로 표시도 태그도 되지 않습니다. 범위 선택은 그 행을 건너뛰고, 그 위에서 `t`를 누르면 그렇다고 알려줍니다.

### 파라미터 {#params}

Sitemap 행에서 `p`를 누르면 그 호스트나 하위 트리로 좁힌 **Params** 서브탭(`Sitemap · Discover · Diff · Params`)이 열립니다. 캡처된 요청에 들어 있는 입력 이름을 엔드포인트, 위치, 이름마다 한 줄씩 보여줍니다. 위치는 `query`, `form`, `multipart`, `json`, `headers`, `cookies`입니다. 각 줄에는 그 이름이 나온 플로우 수, 샘플 값 몇 개가 표시됩니다. 4바이트 이상인 값이 디코딩된 응답 본문의 앞 256 KiB 안에 그대로 다시 나타나면 `↩`가 붙습니다. `↩`는 XSS나 인젝션을 살펴볼 자리라는 표시일 뿐, 취약점 판정은 아닙니다. JSON 이름은 `user.email`, `items[].id` 같은 경로로 표시됩니다. `[]`는 배열 인덱스를 하나로 묶을 뿐 JsonPath 문법이 아닙니다. 모든 브라우저가 보내는 헤더(`User-Agent`, `Accept*`, `Sec-*` 등)는 `a`를 누르기 전까지 빠집니다.

스캔은 Sitemap이 보여주는 플로우를 읽으므로 `/` 쿼리와 `s` 스코프 렌즈가 그대로 적용됩니다. 최신 5,000개를 읽고, 더 오래된 플로우를 읽지 못했으면 헤더에 `TRUNCATED`가 붙습니다. `^R`로 다시 스캔합니다. `Esc`는 노드 필터를 해제하고, 한 번 더 누르면 서브탭으로 돌아갑니다.

| 동작 | 키 | 결과 |
| --- | --- | --- |
| 플로우 열기 | `↵` | 그 이름이 들어 있던 가장 최근 요청을 History 상세로 엽니다 |
| 복사 | `y` / `⇧Y` | 이름 하나, 또는 목록의 모든 이름을 한 줄에 하나씩 |
| 워드리스트 내보내기 | `w` | 목록의 이름(JSON은 마지막 키 이름, 헤더 제외)을 `~/.gori/wordlists/` 아래 파일로 저장합니다 |
...
- 인라인 스크립트는 그 페이지, 또는 페이지의 `<base href>`를 기준으로 해석합니다.
- 외부 스크립트는 캡처된 요청의 `Referer`가 가리키는 페이지를 기준으로 해석합니다. CDN 번들의 `"/api/cart"`는 페이지의 origin을 향하기 때문입니다. `Referer`가 없으면 스크립트 자신의 origin을 쓰고, 그 참조에는 `guessed`가 붙습니다.
- 템플릿 리터럴은 모양을 유지합니다. `` `/api/users/${id}/orders` ``는 디렉터리 `/api/users/`가 아니라 `/api/users/{expr}/orders`가 됩니다.
- 주석 안의 문자열도 남기되(오래된 라우트는 볼 가치가 있습니다) 주석이었다고 표시합니다.
- 바이트는 페이지가 쓴 것으로 다룹니다. 공백은 퍼센트 인코딩하고, CR이나 LF가 든 문자열은 고치지 않고 버립니다. 이미지, 폰트, 맨 `/`는 건너뜁니다.

gori가 한 번도 캡처하지 않은 호스트(번들이 부르는 API 서브도메인도 있지만, SVG 네임스페이스의 `www.w3.org`도 있습니다)는 스코프 include 규칙이 그 호스트를 가리킬 때만 나타납니다. `s` 렌즈를 켜면 참조도 스코프로 걸러집니다.

| 동작 | 키 | `js` 행에서 |
| --- | --- | --- |
| 플로우 열기 | `o` | 그 경로를 참조한 스크립트(또는 페이지)를 History 상세의 응답 창에서 열고, 줄과 바이트 위치를 알려줍니다 |
...
Sitemap 행에서 `⇧E`(또는 `Space` → `E`)를 누르면 OpenAPI 3.0.3 문서를 씁니다. 표시한 경로가 있으면 그 경로들을, 없으면 커서 행을 내보냅니다. 호스트 행은 호스트 전체를, 그 밖의 행은 그 아래 엔드포인트를 내보냅니다. 팝업에서 저장할 경로를 정하는데, 파일 이름이 `.yaml`이나 `.yml`로 끝나면 YAML, 그 밖에는 JSON으로 씁니다. 트리가 보여주는 플로우를 읽으므로 `/` 쿼리, `s` 스코프 렌즈, 정적 파일 숨김 렌즈가 모두 적용되며, gori가 직접 보낸 요청(Repeater, Fuzzer, Discover 등)은 빠집니다. 내보내기는 백그라운드에서 돌고, 끝나면 토스트로 쓴 operation 수, 문서를 자른 상한, 건너뛴 플로우 수(WebSocket, gRPC, SSE, 미완료)를 알려줍니다.

경로는 템플릿으로 바꿔 합치고(`/users/123` → `/users/{userId}`), 파라미터는 모든 샘플에 있을 때만 `required`로 두며, 본문과 응답에는 샘플에서 추론한 스키마가 붙습니다. 자격 증명은 보안 스킴이 되고 값은 쓰지 않으며, TUI 내보내기에는 예시 값이 없습니다. 가린 예시 값, 크기 상한, 모든 옵션은 [`gori run sitemap export`](/ko/reference/cli/#run-sitemap)를 쓰세요. MCP에서는 같은 내보내기를 `export_openapi`로 합니다.
...
트리는 모든 표면에서 제공됩니다. TUI의 History와 Repeater 패널(`p`로 트리와 바이트 미리보기를 전환하며, `^X`는 여전히 바이트 그대로의 덤프를 보여 줍니다), `gori run history show --format json`(`grpc_messages[].protobuf` 필드), 그리고 MCP `get_flow` 도구(`request_grpc_messages` / `response_grpc_messages` 필드)입니다. 잘렸거나 악의적인 메시지는 파싱되는 데까지 디코드된 뒤 거부되는 대신 `complete: false`로 표시됩니다. 어느 쪽이든 원본 옥텟에는 계속 접근할 수 있습니다.
...
- **원본 트리는 사라지지 않습니다.** `gori run history show --format json`과 MCP `get_flow`는 원본 `protobuf` 트리를 그대로 내보내고 그 옆에 `schema`를 더합니다. `^X`는 여전히 바이트 그대로의 덤프를 줍니다.
...
**바이트가 아니라 필드를 편집합니다.** 스키마가 로드되면 Repeater의 gRPC 탭에 같은 페이로드를 보는 두 번째 편집기가 생깁니다. `␣Pf`(요청 카드의 `␣Pf:FIELDS` 칩)가 메시지를 이름과 타입이 붙은 행으로 나열하고, 행에서 `↵`를 누르면 값 입력 칸이 열립니다. 값을 입력해 적용하면 gori는 **그 필드만** 다시 인코딩하고 메시지의 나머지 바이트는 캡처에서 그대로 복사합니다. 이걸 가능하게 하는 게 선언입니다. 와이어는 어떤 필드가 varint라는 것만 말할 뿐, 그것이 `int32`(부호 확장)인지 `bool`인지 `enum`인지 zigzag `sint32`인지는 말하지 않고, `-3`은 넷 각각에서 서로 다른 옥텟이기 때문입니다. `bytes`는 hex로, enum은 자기 값 이름으로, packed 런은 쉼표로 구분한 목록으로 편집합니다. 값을 바꾸지 않고 적용하면 캡처가 바이트 그대로 돌아옵니다. `^X`와 같은 이유로 unary 호출에만 적용됩니다. 메시지가 0개거나 여러 개인 본문에는 편집할 단일 페이로드가 없습니다. 중첩 메시지는 그 안의 필드 단위로 편집할 수 있고, 메시지 자체의 행은 편집 대상이 아닙니다.

**입력할 수 없는 행은 그 이유를 말합니다.** `(undeclared)` 필드 번호와 스키마가 부정하는 와이어 타입은 둘 다 읽기 전용으로 남고 원본 해석을 유지합니다. 무엇**으로** 입력해야 할지가 없고, 거기에 타입 편집기를 내미는 것이야말로 이 렌즈가 피하려고 존재하는 추측이기 때문입니다. 그 옥텟을 바꾸는 방법은 여전히 `^X`이고, 스키마가 불가능하다고 말하는 것을 보내는 방법도 여전히 `^X`입니다. 편집된 메시지 앞의 5바이트 길이 접두사는 hex 편집 때와 똑같이 `␣Pr:FRAME` 토글이 관장합니다.

**대상에게 직접 물어볼 수도 있습니다.** gRPC **서버 리플렉션**에 응답하는 서버는 이미 디스크립터를 갖고 있습니다. `gori run grpc reflect https://api.test:443`이 그것을 받아와 프로젝트에 캐시하고, 에이전트는 MCP `grpc_reflect`로 같은 일을 합니다. TUI에서는 캡처된 flow 위에서 실행합니다. 팔레트(`Ctrl-P`)의 **gRPC: reflect schema**이며, 그 행의 호스트를 대상으로 합니다. `grpc.reflection.v1`을 먼저, `v1alpha`를 그다음으로 시도하고(실제 배포된 서버 대부분은 아직 v1alpha입니다), 둘 다 응답하지 않으면 조용히 실패하지 않고 그렇다고 말합니다. 서비스 목록 → 각 서비스를 선언한 파일 → 그 파일들의 import 순으로, 그래프가 닫힐 때까지 따라갑니다.
...
```text
status:5xx                  오류로 끝난 플로우
host:api.example.com        호스트 하나
method:POST body:password   "password"가 들어간 POST 요청
dur:>500                    500 ms보다 느린 응답
path~/admin/                정규식에 매칭되는 경로
```
...
**뷰(view)**는 목록을 좁히는 이름 붙은 쿼리이며, 무엇보다 **모드**입니다. `v`를 눌러 하나를 고르면, 그 뒤로 다른 필터를 입력하는 동안에도 계속 좁혀 줍니다. 이것이 뷰와 필터 바의 차이입니다. 뷰는 입력한 내용을 *대체하지 않고* 그 위에 AND로 얹힙니다. `s` 스코프 렌즈와 같은 방식이라, `/ status:5xx`는 뷰를 지우는 대신 뷰 안에서 다시 좁힙니다. 필터 줄의 `v:name` 칩이 지금 무엇을 보고 있는지 항상 알려 줍니다. 옆의 `⌁follow`·`s scope` 칩과 같이 소문자로 그려지고, 줄에 비해 이름이 긴 뷰는 줄여서 씁니다(`History + Repeater`는 `v:history+rptr`). 이름이 이름으로 쓰이는 곳(피커, `gori run views`, `--view`, MCP)에서는 그대로이고, `--view`는 대소문자를 가리지 않아 `--view history`로 `History`를 찾습니다.
...
## 컬럼 (`Space` `Z` `c`) {#columns}

쿼리는 *어떤 플로우가 걸리는가*에 답합니다. **컬럼(column)**은 나머지 절반, 즉 *각 행에서 X의 값은 무엇인가*에 답합니다. `Space` `Z` `c`를 누르면 컬럼을 추가할 수 있습니다(`X-Request-Id`, JWT의 `sub`, rate-limit 헤더, JSON 본문 속 필드). 그러면 그 값이 목록의 모든 플로우 옆에 그려집니다.
...
따라서 `/ status:5xx` → `Shift-T` → `Space` → `d`면 모든 오류 플로우를 확인 한 번으로 삭제하고, `y`면 그 URL들을 한꺼번에 복사합니다. 메뉴 제목이 `SPACE · 3 MARKED`로 바뀌고 항목 이름도 `Delete 3 flows`, `Mine 3 flows`처럼 바뀌므로, 일괄 처리가 예상 밖에 일어나는 일은 없습니다.
...
| 연결(Link…) | `Space` `L` | 한 카드에 모든 이슈와 노트(그리고 `+ New issue…` / `+ New note…`)가 함께 뜹니다. 대상은 한 번만 고르거나 새로 만들고, 전체를 첨부. 이슈에 붙이면 각 교환을 변경 불가 증거로 동결까지 합니다(사본은 최대 20개, 그 이상이면 링크만) |
...
표시는 필터 변경, 정렬 변경, 다른 탭에 갔다 오는 것에도 유지됩니다. 카운트 칩이 현재 화면에 없는 개수를 알려줍니다. 트래픽을 보내는 동작은 여전히 먼저 확인을 받고 요청마다 스코프를 검사합니다. 표시는 요청 수를 바꾸지, 게이트를 바꾸지 않습니다. 플로우 하나에만 의미가 있는 몇 가지(상세 보기 열기, Sequencer, 브라우저로 응답 열기, 응답 모킹)는 단일 대상으로 남으며, 표시가 있는 동안 앞의 세 가지는 메뉴 항목에 `(cursor)`가 붙습니다.
...
터미널은 페이지를 레이아웃하지도, PNG를 보여주지도, PDF를 페이지 단위로 넘기지도 못합니다. `Space` `>` `b`(**Send flow to…** → **Open response in browser**)는 그걸 할 수 있는 곳으로 응답을 넘깁니다. gori가 **디코딩된** 본문을 `~/.gori/preview/` 아래 파일로 쓰고, 데스크톱의 오프너(macOS는 `open`, Linux는 `xdg-open`)로 엽니다.

같은 동작이 세 곳에 있습니다.

| 위치 | 키 | 여는 대상 |
|------|-----|----------|
| History 목록 | `Space` `>` `b` | 커서 행의 응답 |
| History 상세 | `Space` `>` `b` | 열려 있는 플로우의 응답 |
| Repeater | `Space` `>` `b` | 활성 서브탭의 마지막 응답 |
...
**HTML은 실행됩니다.** 타깃의 페이지를 브라우저로 열면 그 자바스크립트가 실행됩니다. 타깃의 쿠키에는 닿을 수 없는 `file://` 오리진에서 실행되며, Burp의 "Show response in browser"와 같은 거래입니다. gori는 스크립트를 제거하지 않습니다. 무력화된 렌더링은 테스트 대상과 다른 문서이기 때문입니다. 브라우저가 실행할 문서일 때는 상태 줄이 그렇게 말해줍니다. 맨 글자 키 없이 메뉴(`Space` `>` `b`)에만 있는 이유이기도 합니다.

미리보기 디렉터리는 gori의 것이고, 모드는 `0700`이며, 쓸 때마다 최신 32개만 남기고 정리됩니다. `~/.gori`를 지우면 함께 사라집니다.

## Match & Replace (Rewriter 탭) {#match-replace}

**Rewriter** 탭이 Match & Replace 편집기입니다. 오가는 요청/응답을 그 자리에서 재작성하는 규칙을 관리합니다. 기본적으로 탭 바 밖에 있으니 **`0`**을 누르고 "rewriter"를 입력하거나, 커맨드 팔레트(`Ctrl-P` → **Match & Replace** 또는 **Go to Rewriter**)를 쓰거나, Preferences에서 슬롯을 주세요.
...
| **Short circuit**(편집기에서는 `stub`) | 원본에 접속하지 않고 규칙에 적힌 응답으로 답함 |
| **Pipe** | 명령을 (셸 없이) 실행해 매칭된 바이트를 stdin으로 넘기고, 그 stdout으로 바꿔 끼움 |
...
어떤 규칙이든 **호스트**로 범위를 좁혀 매칭되는 트래픽에만 적용할 수 있습니다. 일반 문자열은 그 호스트와 그 서브도메인에 매칭되고(`example.com`은 `example.com`과 `api.example.com`에 매칭되지만 `xexample.com`이나 `example.com.evil.net`에는 매칭되지 않습니다), 더 넓게 잡으려면 `*`를 명시적으로 씁니다(`*.example.com`). 비워 두면 모든 호스트에 적용됩니다.
...
규칙을 만들 때는 편집기의 `scope:` 행에서 고르고, 이미 있는 규칙은 목록에서 `Space` → `s`로 두 곳 사이로 옮깁니다. 필드와 지금 이 프로젝트에서의 켜짐/꺼짐 상태는 그대로 따라가고, 달라지는 것은 누가 이 규칙을 보느냐입니다.
...
- `t`는 **이 프로젝트에서** 규칙을 켜고 끕니다. 전역 규칙이라면 오버라이드가 기록되고 행에 `G*` 표시가 붙습니다.
...
**part**를 `ws`로 두면 규칙이 HTTP 헤드나 본문 대신 WebSocket 메시지를 재작성합니다. **방향은 target이 정합니다.** `request`는 클라이언트 → 서버, `response`는 서버 → 클라이언트입니다. 나머지는 그대로입니다. 리터럴이나 정규식, 캡처 그룹, `$BIND.NAME` 바인딩, 그리고 호스트 글롭이 똑같이 동작하며, 글롭은 그 소켓을 연 호스트와 매칭됩니다.
...
나머지 동작은 이미 존재하는 메시지를 재작성합니다. **Short circuit**은 대신 답합니다. 요청이 매칭되면 gori가 직접 작성한 응답을 돌려주고, 원본에는 접속하지 않습니다. Replace 규칙으로는 구조적으로 불가능한 경우를 덮습니다. 엔드포인트가 404·500을 내거나, 원본이 꺼져 있거나 인증 벽 뒤에 있거나, 본문을 유도하는 게 아니라 만들어 내야 하는 경우입니다.
...
### 모킹: 디렉터리, 캡처한 응답, 장애 주입 {#mocking}

Short circuit 규칙의 `source:` 행은 답을 어디서 가져올지 고릅니다. `inline`과 `body file`은 위에서 설명한 스텁입니다. 나머지 넷을 쓰면 작은 모킹 도구가 됩니다.

| Source | 답 |
|--------|----|
| **directory** | 요청 경로가 가리키는 파일을 로컬 디렉터리에서 제공(Map Local) |
| **close** | 응답 없음. 연결을 닫습니다(FIN) |
| **reset** | 응답 없음. 연결을 리셋합니다(TCP RST) |
| **hang** | 응답 없음. 클라이언트가 포기하거나 규칙의 상한에 이를 때까지 연결을 붙잡습니다 |

`options:`를 누르면 source별 설정이 열립니다. 모든 source는 **delay**를 받습니다. 답(또는 장애)을 보내기 전에 그만큼 기다리므로, 느린 엔드포인트도 규칙 하나로 만들 수 있습니다.

**Map Local.** `dir:`에 디렉터리를, **strip prefix**에 그 디렉터리가 대신할 URL 접두사를 지정합니다. `/static/`이라면 `/static/js/app.js` 요청에 `<dir>/js/app.js`로 답하고 쿼리 문자열은 무시합니다. `/`로 끝나는 경로는 `index.html`을 제공하며, 디렉터리 목록은 만들지 않습니다. 이때 `response:` 행은 선택 사항인 헤드 템플릿이 되고(기본 `200 OK`), `Content-Type`이 없으면 파일 확장자로 찾아 넣습니다. 클라이언트 측 검사를 없앤 JavaScript나, 매번 로드되는 설정 JSON을 뒤집은 버전을 제공할 때 씁니다.

요청 경로는 클라이언트의 바이트가 로컬 파일을 가리키는 것이므로 가둡니다. 퍼센트 디코딩은 한 번만 하고, 그 뒤 `.`/`..` 세그먼트, 점 파일, 백슬래시, NUL은 거절합니다. 풀어낸 파일은 디렉터리의 실제 경로 아래에 있어야 하므로 심볼릭 링크로도 밖으로 나갈 수 없습니다. 거절된 경로에는 `X-Gori-Short-Circuit: error`를 단 `404`로 답하고 기록합니다. 파일이 그냥 **없으면** 같은 방식의 `502`로 답합니다. 단, 규칙이 **fall through**를 켜 두었다면 요청은 원본으로 넘어갑니다. 규칙이 매칭한 요청이 원본에 닿는 유일한 경우이며, 무엇이든 다이얼하기 전에 결정됩니다. 디렉터리 자체가 없으면 언제나 `502`입니다.

**Mock this response.** History의 flow에서 `Space` `M`(flow 상세에서도 가능)을 누르면, 캡처된 응답으로 초안을 잡은 short-circuit 규칙이 규칙 폼에 열립니다. flow의 호스트, 요청 줄에 고정한 정규식(`\AGET /api/me(\?| )`, 그래서 `/api/mes`는 잡지 않습니다), 그리고 인라인 스텁으로 넣은 응답입니다. 본문은 편집할 수 있게 디코딩하며(gzip, chunked) `Content-Encoding`도 함께 뺍니다. 폼을 저장하기 전에는 아무것도 저장되지 않습니다. 참조가 아니라 복사본이므로 나중에 flow를 지워도 바뀌는 것이 없습니다. 캡처 본문이 잘린 flow, 완전한 응답이 없는 flow, gori가 직접 답한 flow, 바이너리 본문은 이유와 함께 거절됩니다. 바이너리 본문은 파일로 저장한 뒤 body file로 쓰세요.

**장애 주입.** `close`는 요청 본문을 다 읽은 뒤 한 바이트도 보내지 않고 닫습니다. `reset`은 `SO_LINGER 0`으로 닫아 클라이언트가 TCP 리셋을 보게 합니다(TLS 터널 안에서도 앞에 `close_notify`가 붙지 않습니다). `hang`은 클라이언트가 포기하거나 `hang ms`(기본 30초)가 지날 때까지 연결을 붙잡습니다. 로그인이나 결제 단계에 걸어 두면 백엔드가 죽었을 때 클라이언트가 어떻게 하는지 볼 수 있습니다. flow는 장애 종류와 그것을 주입한 규칙과 함께 `Aborted`로 기록됩니다. delay와 hang은 120초가 상한이고, 동시에 붙잡는 연결은 최대 256개입니다. 이를 넘으면 hang은 바로 닫고 delay는 건너뛰며, flow에 그렇게 남습니다.

**어느 규칙이 답했는지**도 flow에 기록됩니다. 상세에는 `answered by gori — project rule #4 · dir js/app.js`로 나오고, `gori run history`는 `[stub` 옆에, MCP는 `source_ref`로 돌려줍니다. 규칙을 고치거나 지워도 남습니다.

하지 않는 것: `text/event-stream` 본문은 한 번 보내고, 여느 스트림이 끝났을 때처럼 클라이언트가 다시 연결합니다. gRPC는 HTTP/2가 필요하므로(아래) 모킹할 수 없습니다. `101` 응답은 거절합니다.
...
탭은 **기본적으로 바 위에 없습니다.** `0`으로 열 수 있고, `settings:tabs`에서 Rewriter 옆에 슬롯을 줄 수 있습니다.
...
두 스타일은 공존합니다. 건너뛰고 싶은 노이즈에는 `strip`을, 절대 놓치면 안 되는 행에는 `full`을 쓰면 됩니다. 색상은 6가지(`red`, `orange`, `yellow`, `green`, `blue`, `purple`)이며 활성 테마 팔레트로 해석되므로 밝은 테마와 어두운 테마 양쪽에서 제대로 읽힙니다. 탭의 사용자 색상 패널에서 이름 붙인 hex 색상을 직접 추가할 수도 있으며, 이 색상은 전역이고 테마를 따르지 않습니다.
...
조건은 History 필터 바가 쓰는 것과 같은 쿼리 언어입니다. 필터 바에 있는 모든 필드에 `AND` / `OR` / `NOT`, `-부정`, `(그룹)`을 더한 형태이며, `when:` 행에서 `Tab`을 누르면 캐럿 아래 토큰이 자동완성됩니다. History 검색 바와 다르게 동작하는 세 가지가 있고, gori는 빈 목록을 보고 나서야 알아채는 대신 거부하거나 경고합니다.

- **행이 답할 수 없는 항목은 저장소에서 매칭합니다.** `body:`, `header:`, `scope:`는 그리는 중인 행에 없으므로, 이런 필드를 쓴 규칙은 다시 그릴 때마다 프로젝트 데이터베이스에 묶음 쿼리 한 번으로 해석됩니다. `body:`는 캡처된 바이트를 방향마다 64 KiB씩 읽습니다. History 자체의 `body:` 인덱스가 덮는 범위의 8배이므로, 컬러 규칙은 그 발상을 준 쿼리보다 더 넓게 봅니다. 다만 그 상한을 넘는 매칭은 여전히 놓칩니다.
- **`host:`는 DNS 레이블 글롭이 아니라 부분문자열입니다.** `host:alpha.test`는 `xalpha.test`도 매칭합니다.
- **조건은 저장할 때 검사합니다.** 모르는 필드, 잘못된 정규식, 모든 플로우에 매칭되는 조건, 그 필드가 받지 않는 값은 모두 이유와 함께 거부됩니다. 그냥 두면 `hsot:evil.com`은 조용히 자유 텍스트 검색이 되어 규칙이 영원히 발동하지 않고, 항목 하나가 빠진 규칙은 적어 둔 것보다 더 많이 칠하게 됩니다.

규칙은 Match & Replace 규칙과 똑같이 이 프로젝트 또는 모든 프로젝트가 읽는 **전역 라이브러리** 중 한쪽에 삽니다. `Space → s`로 둘 사이를 옮기고, `t`로 여기서 켜고 끄고, `Space → T`로 전역 규칙의 기본값을 모든 곳에서 뒤집습니다. 라이브러리와 의견이 다른 프로젝트는 그 *불일치만* 저장하며, 다시 의견이 같아지는 순간 불일치는 삭제됩니다. 껐다 켠 규칙이 오늘의 답에 고정되는 대신 라이브러리를 다시 따라가게 되는 이유입니다.

| 동작 | 키 |
|--------|-----|
| 규칙 추가 / 편집 | `a` / `Enter` 또는 `e` |
| 여기서 켜기·끄기 | `t` |
| 전역 규칙의 기본값을 모든 곳에서 뒤집기 | `Space → T` |
| 프로젝트 ⇄ 전역 이동 | `Space → s` |
...
- 평범한 **Match & Replace 규칙**이 그 값을 내보냅니다. `set header` 규칙의 치환값이나 본문 `replace`에 쓴 `$BIND.SESSION`은 규칙을 저장할 때가 아니라 요청이 나갈 때 해석됩니다.

한 이름은 정확히 하나의 extract 규칙이 씁니다. 같은 이름을 주장하는 두 번째 규칙은 저장 시점에 이유와 함께 거부됩니다. 선언되었지만 아직 바인딩되지 않은 이름은 빈 값으로도, 리터럴 `$BIND.SESSION`으로도 나가지 **않습니다**. 규칙이 적용되지 않고 그 이유가 이벤트 피드에 남습니다.
...
| **Import: cURL** | 붙여넣은 curl 명령("Copy all as cURL"처럼 여러 개도 가능) → URL마다 요청 하나 |

형식이 잘못된 항목은 전체 임포트를 중단시키지 않고 건너뜁니다. OpenAPI 3.x와 Swagger 2.0 스펙은 로컬 JSON Pointer `$ref`를 해석하고, 원격 참조는 보고만 할 뿐 가져오지 않습니다. 임포트된 플로우는 캡처된 트래픽처럼 History에 들어오므로, 똑같이 필터링하고 Repeater로 재전송하거나 퍼징·스캔할 수 있습니다.
...
**cURL**은 경로 입력 대신 붙여넣기 상자를 엽니다. 각 명령은 [Repeater의 Paste cURL](/ko/guide/repeater-and-fuzzer/#repeater)과 같은 방식으로 읽히고, 요청은 만들어진 그대로 저장되므로 더 긴 `-d` 옆의 `-H 'Content-Length: 1'`은 입력한 desync 그대로 남습니다.
...
그래서 **전송 하나 또는 실행 하나**가 자기 지문을 지정할 수 있고, dial 시점에 해석되며, 목적지 테이블은 그대로 둡니다. Repeater에서 플로우를 열고 TARGET 밴드가 `␣Pt:chrome`이 될 때까지 `␣Pt`를 눌러 전송하세요. 그 다음 탭을 복제해 `curl`로 돌린 뒤 다시 전송합니다. 탭 둘, 호스트 하나, 진짜 핸드셰이크 둘, 응답 둘이 화면에 나란히 남습니다. 헤드리스로는 `gori run repeater 42 --tls-preset chrome`이고, 스윕 전체는 `gori run fuzz --tls-preset chrome`으로 지정합니다.
...
| **Env** | 아웃바운드 요청을 위한 프로젝트별 `$ENV.KEY` 변수. [Repeater & Fuzzer](/ko/guide/repeater-and-fuzzer/#environment-variables) 참고 |
| **Project settings** | scope 렌즈 + **샌드박스** 토글, 전역 Settings 기본값을 재정의하는 프로젝트별 네트워크 고정(bind, 업스트림 프록시, 타임아웃, 캡처 한도), 그리고 gRPC [`.proto` 스키마](#proto-schema) 경로 |
| **Activity** | 이 프로젝트에 **누가 무엇을** 바꿨는지. 추가 전용 이벤트 피드를 최신순으로. 설정 변경(스코프 규칙, 샌드박스, host override, `$ENV.KEY` 변수, rewrite 규칙, 네트워크 고정)은 어디서 바꾸든 기록되고, 각 행이 그것을 만든 **주체**(`tui` · `cli` · `agent`)를 밝힙니다. 백그라운드 작업 결과와 에이전트 툴 호출도 함께 들어오고, 붙어 있는 에이전트에게 보낸 오퍼레이터 메시지도 마찬가지입니다 — source `operator`, 전달마다 한 행, 어느 경로로 나갔는지(또는 왜 못 나갔는지) 이름을 붙여서. `s` source, `l` level, `a` actor, `/` 텍스트로 필터하고 `↵`로 이벤트가 가리키는 플로우·세션을 열며, `⇧X`로 피드를 비웁니다(에이전트 감사 기록도 함께 사라지므로 먼저 확인을 묻습니다). `⇧X`는 History·Probe 이슈·Issues 목록·Authorize 큐를 비우는 키와 같으며, 각각 자기 탭 안에서만 동작합니다. 소문자 `c`는 다른 곳과 똑같이 캡처 토글로 남습니다. 알림을 띄우지 *않고* 실패한 훅이나 세션 바인딩이 보이는 곳입니다 |
