...
gori는 가능한 곳에서는 커널에서 목적지를 복구하고(Linux는 `SO_ORIGINAL_DST`, macOS는 root 가 필요한 `pf` 조회), 그렇지 않으면 클라이언트의 `Host` 헤더나 TLS SNI 에서 가져옵니다. 어느 쪽을 썼는지는 로그에 남으므로, 목적지가 이상해 보이면 따라갈 수 있습니다.
