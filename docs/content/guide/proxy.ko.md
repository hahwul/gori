...
프록시는 gori의 핵심입니다. 클라이언트와 업스트림 서버 사이에 자리 잡아 오가는 요청과 응답을 *플로우*로 기록하고 현재 프로젝트에 저장합니다. **History**는 그 플로우를 되짚어 읽는 곳입니다.
...
**샌드박스**는 테스트를 허용된 범위 안으로만 강하게 가두는 차단 게이트입니다. **Project** 탭의 **NETWORK** 패널에서 토글합니다(기본값: **off**). **on**인 동안 캡처 프록시는 스코프가 *허용*하는 요청만 전달하고, 나머지는 **오리진에 닿기 전에 차단**합니다(클라이언트는 `X-Gori-Sandbox: blocked` 헤더가 붙은 `403`을 받습니다). 차단된 시도도 aborted 플로우로 기록되어 History에서 확인할 수 있습니다. 여기서 "허용"이란 스코프를 **허용 목록(allowlist)**으로 평가한 것으로, **include** 규칙이 하나 이상 매칭되고 **exclude** 규칙에는 매칭되지 않아야 합니다.
...
**Sitemap** 탭은 History를 중복 제거된 `host → path` 엔드포인트 트리로 접어, 메서드 칩과 스코프 마커를 함께 보여줍니다. 대상의 공격 표면을 가장 빠르게 파악하는 방법입니다. 숫자로 된 경로 세그먼트는 하나로 접을 수 있어 `/user/1`과 `/user/2`가 한 노드를 공유합니다.
...
gori는 단순한 바이트 파이프가 아니라 프로토콜을 인식합니다.
...
와이어 프로토콜 위에서, gori는 흔히 쓰이는 페이로드를 인라인으로 디코드하므로 다른 도구를 꺼낼 필요가 없습니다.
