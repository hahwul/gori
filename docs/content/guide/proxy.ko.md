...
요청은 HTTP/1.1 과 HTTP/2 에서 붙잡히며 gRPC 도 포함됩니다. 응답도 마찬가지지만, 마지막 바이트를 기다릴 수 없는 응답은 예외입니다. WebSocket 업그레이드(`101`), Server-Sent Events 스트림, close 로 구분되는 응답은 붙잡지 않고 도착하는 대로 흘려보냅니다. 업그레이드 이후의 WebSocket 메시지는 중계되고 캡처될 뿐 붙잡히지 않으므로, 한 번 열린 소켓은 일시 정지하거나 드롭할 수 없고 메시지가 편집기에 올라오지도 않습니다. 그 소켓을 연 요청은 다른 요청과 똑같이 붙잡을 수 있습니다. 열려 있는 소켓의 메시지라도 [Match & Replace 규칙](#match-replace-websocket)은 재작성할 수 있습니다. 흐르는 중에 적용되는 규칙이지 붙잡는 것이 아닙니다.
...
| **WebSocket** | 실시간 메시지 캡처, 리피터, 메시지 Match & Replace. 핸드셰이크에서 압축이 제거됩니다(아래 참고) |
...
WebSocket 메시지는 여전히 **붙잡을** 수 없습니다 — [무엇이 붙잡히는가](#what-gets-held)를 보세요. 규칙은 흐르는 중에 재작성할 뿐 아무것도 멈추지 않습니다.
