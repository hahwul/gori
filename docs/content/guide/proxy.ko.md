...
| 동작 | 키 | 표시된 경로에 대해 |
|------|-----|------------------|
| 경로 태그 | `Space` `T` | 편집기 하나, 메모 하나를 표시된 모든 경로에 적용(비우면 전체 해제) |
...
규칙은 Match & Replace 규칙과 똑같이 이 프로젝트 또는 모든 프로젝트가 읽는 **전역 라이브러리** 중 한쪽에 삽니다. `s`로 둘 사이를 옮기고, `x`로 여기서 켜고 끄고, `Space → T`로 전역 규칙의 기본값을 모든 곳에서 뒤집습니다. 라이브러리와 의견이 다른 프로젝트는 그 *불일치만* 저장하며, 다시 의견이 같아지는 순간 불일치는 삭제됩니다. 껐다 켠 규칙이 오늘의 답에 고정되는 대신 라이브러리를 다시 따라가게 되는 이유입니다.
...
그래서 **전송 하나 또는 실행 하나**가 자기 지문을 지정할 수 있고, dial 시점에 해석되며, 목적지 테이블은 그대로 둡니다. Repeater에서 플로우를 열고 TARGET 밴드가 `␣P:chrome`이 될 때까지 `␣P`를 눌러 전송하세요. 그 다음 탭을 복제해 `curl`로 돌린 뒤 다시 전송합니다. 탭 둘, 호스트 하나, 진짜 핸드셰이크 둘, 응답 둘이 화면에 나란히 남습니다. 헤드리스로는 `gori run repeater 42 --tls-preset chrome`이고, 스윕 전체는 `gori run fuzz --tls-preset chrome`으로 지정합니다.
