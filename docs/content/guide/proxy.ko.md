...
`i`를 눌러 **Intercept**를 켭니다. 켜져 있으면 매칭되는 요청(그리고 선택적으로 응답)이 붙잡혀, 계속 진행하기 전에 포워드, 드롭, 편집할 수 있습니다. Intercept 탭 상단의 필터 바에서 붙잡을 방향을 고르고 쿼리 언어 표현식으로 붙잡을 대상을 좁힐 수 있어, 관심 있는 트래픽에서만 멈춥니다. 이 탭에서는 `c`가 붙잡을 방향을 순환하고(전체 / 요청만 / 응답만) `/`가 조건을 편집합니다. 캡처 토글 `c`는 다른 모든 탭에서 그대로입니다. 인터셉트를 **끄면** 아직 붙잡혀 있던 것이 전부 원래 바이트로 흘러가므로, 포워드하지 않은 편집은 버려집니다.
...
- **텍스트 메시지를 편집하면 줄 끝이 바뀝니다.** 편집기가 `CRLF`를 `LF`로 정규화하며, 리피터와 HTTP 인터셉트 편집기도 같습니다. WebSocket 페이로드에서는 `$BIND.NAME` 바인딩이 확장되지 *않습니다*. 거기서 `$`는 참조가 아니라 바이트이기 때문입니다. 입력한 그대로 전송됩니다.
...
스캔은 트리가 보여주는 플로우(`/` 쿼리와 렌즈 적용)를 최신 것부터 한 번에 500개씩, 아직 읽지 않은 응답만 읽습니다. 그래서 더 둘러본 뒤 다시 돌리면 새 번들만 읽습니다. 끝나면 토스트로 새로 찾은 엔드포인트 수와 걸린 상한(본문은 2 MiB까지 읽습니다)을 알려줍니다. `Space` → `U`는 `js` 행을 숨기거나 보여줍니다.
...
따라서 `/ status:5xx` → `Shift-T` → `Space` → `d`면 모든 오류 플로우를 확인 한 번으로 삭제하고, `Space` → `Y`면 그 URL들을 한꺼번에 복사합니다. 메뉴 제목이 `SPACE · 3 MARKED`로 바뀌고 항목 이름도 `Delete 3 flows`, `Mine 3 flows`처럼 바뀌므로, 일괄 처리가 예상 밖에 일어나는 일은 없습니다.
...
표시는 필터 변경, 정렬 변경, 다른 탭에 갔다 오는 것에도 유지됩니다. 카운트 칩이 현재 화면에 없는 개수를 알려줍니다. 트래픽을 보내는 동작은 여전히 먼저 확인을 받고 요청마다 스코프를 검사합니다. 표시는 요청 수를 바꾸지, 게이트를 바꾸지 않습니다. 플로우 하나에만 의미가 있는 몇 가지(상세 보기 열기, Sequencer, 브라우저로 응답 열기, 응답 모킹)는 단일 대상으로 남으며, 표시가 있는 동안 메뉴 항목에 `(cursor)`가 붙습니다.
...
**cURL**은 경로 입력 대신 붙여넣기 상자를 엽니다. 각 명령은 [Repeater의 Paste cURL](/guide/repeater-and-fuzzer/#repeater)과 같은 방식으로 읽히고, 요청은 만들어진 그대로 저장되므로 더 긴 `-d` 옆의 `-H 'Content-Length: 1'`은 입력한 desync 그대로 남습니다.
