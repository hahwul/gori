...
요청은 HTTP/1.1 과 HTTP/2 에서 붙잡히며 gRPC 도 포함됩니다. 응답도 마찬가지지만, 마지막 바이트를 기다릴 수 없는 응답은 예외입니다. WebSocket 업그레이드(`101`), Server-Sent Events 스트림, close 로 구분되는 응답은 붙잡지 않고 도착하는 대로 흘려보냅니다. 업그레이드 이후의 WebSocket 메시지는 프레임 단위로 중계되고 캡처될 뿐 붙잡히지 않으므로, 한 번 열린 소켓은 일시 정지하거나 편집하거나 드롭할 수 없습니다. 그 소켓을 연 요청은 다른 요청과 똑같이 붙잡을 수 있습니다.
