...
Filter results with ffuf-style matchers and filters on status, size, words, lines, round-trip time (`--mt`/`--ft`, in ms, the dimension a time-based blind payload is the only evidence for), and body regex (headless also a response-head substring with `--mh`/`--fh` and gRPC status with `--mg`/`--fg`), plus auto-calibration to drop noisy baselines. Auto-calibration samples the target several times before the sweep and compares each response against every sampled shape, widened by the jitter those samples themselves showed, so a page carrying a per-request id or timestamp calibrates out, while a target whose samples were identical is still compared exactly. Matched responses are highlighted and can be extracted with a capture regex.

The ADVANCED card also holds the run-shaping rows. **Stop after N hits** and **Stop on (DIM:SPEC)** end a sweep early once the matchers have hit N times or a response meets one condition; a run that stops this way ends `condition_met` rather than `stopped`. **Keep interesting only** makes a saved run keep just the matched, errored and stopping rows. **Race (N conns)** replaces the payload sweep with N copies of the request released together (last-byte sync), and **Max requests** caps the true wire count. Headless these are `--stop-after-matches`, `--stop-on`, `--keep`, `--race` and `--max-requests`; see the [CLI Reference](/reference/cli/#run-fuzz).
