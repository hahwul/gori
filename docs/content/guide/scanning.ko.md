...
링크는 포인터일 뿐이고, 가리키는 대상은 둘 다 변합니다. Repeater 탭은 응답을 하나만 들고 있어 다음 전송이 그것을 덮어쓰고, History 플로우는 보존 정리 한 번이면 `(gone)`이 됩니다. 정확한 바이트가 중요할 때, 즉 취약점을 확인해 준 그 응답을 수정 후 재테스트하는 동안에도 그대로 남겨야 할 때는 **freeze**하세요. History, History 상세, Repeater에서 `Space` → **Link & freeze…**는 같은 피커를 이슈만 보여 주는 모드로 엽니다. `↵`는 교환을 링크하면서 요청·응답·상태·소요 시간·프로토콜·오류·잘림 여부를 한 트랜잭션으로 이슈에 복사하고, `+ New issue…`는 이슈 생성과 동결을 한 번에 끝냅니다. 이미 링크된 이슈에서는 RELATED 카드의 live 행에서 `f`(또는 `Space` → **Manage links** 안에서 `f`)로 그 링크를 동결합니다. RELATED는 **LIVE** 행과 **FROZEN** 행을 구분해 보여 주고, 동결된 행은 출처(`hist #12`, `repeater #3`)·복사한 시각·상태·크기를 표시하며, 그 위에서 `↵`를 누르면 History 드릴인이 아니라 상세 위의 읽기 전용 뷰어가 열립니다(`↹`로 요청/응답 전환, `y`로 보이는 쪽 복사). 거기서는 live 플로우에 아무 것도 할 수 없고 `esc`는 원래 행으로 돌아옵니다. 이슈는 복사본을 원하는 만큼 가질 수 있어(수정 전 하나, 재테스트 후 하나) 각각 저장된 요청·응답의 SHA-256을 가지며, Markdown·JSON 내보내기와 MCP `get_issue`는 이를 출처 정보로 실어 나릅니다(바이트는 싣지 않습니다). 복사본은 정리되거나 덮어써지지 않으며, `Space` → **Delete frozen evidence**(확인 후) 또는 이슈 삭제로만 사라집니다. 한 번도 보내지 않은 Repeater 탭은 요청만 동결하지 않고 거부하며, 응답이 아직 도착하지 않은 플로우도 마찬가지입니다. 1 MB를 넘는 복사본은 먼저 바이트 비용을 보여 주고, 프로젝트의 증거 총량은 256 MB로 제한됩니다. Repeater 복사본에 대해 알아둘 두 가지: 탭에 *저장된* 요청(바인딩은 펼치지 않은, 탭이 들고 있는 그대로)과 저장소에 남은 마지막 응답을 짝지어 복사합니다. 저장소는 성공한 전송의 응답만 남기므로, 취약점을 확인해 준 전송 직후, 둘이 일치할 때 동결하세요. 또 WebSocket 탭이나 플로우의 복사본은 핸드셰이크이며 프레임 기록은 복사되지 않습니다. History 상세의 통계 줄과 Repeater의 RESPONSE 테두리에는 열린 플로우나 탭의 복사본이 생기면 `frozen ×N`이 표시됩니다. 복사본이 있다는 표시이지, 플로우나 탭이 잠겼다는 뜻은 아닙니다. 탭은 그대로 편집·전송할 수 있고 다음 전송이 응답을 덮어쓰는데, 바로 그래서 복사본을 떠 둔 것입니다.
