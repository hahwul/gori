...
링크는 포인터일 뿐이고, 가리키는 대상은 둘 다 변합니다. Repeater 탭은 응답을 하나만 들고 있어 다음 전송이 그것을 덮어쓰고, History 플로우는 보존 정리 한 번이면 `(gone)`이 됩니다. 정확한 바이트가 중요할 때는 **freeze**하세요. History, History 상세, Repeater에서 `Space` → **Link & freeze…**는 같은 피커를 이슈만 보여 주는 모드로 엽니다. `↵`는 교환을 링크하면서 요청·응답·상태·소요 시간·프로토콜·오류·잘림 여부를 한 트랜잭션으로 복사하고, `+ New issue…`는 이슈 생성과 동결을 한 번에 끝냅니다. 이미 링크된 이슈에서는 RELATED 카드의 live 행에서 `f`(또는 `Space` → **Manage links** 안에서 `f`)로 그 링크를 동결합니다. RELATED는 **LIVE** 행과 **FROZEN** 행을 구분해 보여 줍니다. 동결된 행에는 출처(`hist #12`, `repeater #3`)·복사한 시각·상태·크기가 표시되고, `↵`를 누르면 읽기 전용 뷰어가 열립니다(`↹`로 요청/응답 전환, `y`로 보이는 쪽 복사).
...
각 스냅숏에는 저장된 요청과 응답의 SHA-256이 있습니다. 보존 정리나 재전송으로 바뀌지 않으며, 스냅숏 자체를 명시적으로 삭제해야만 사라집니다. 한 번도 보내지 않은 Repeater 탭과 응답이 아직 도착하지 않은 플로우는 동결할 수 없습니다. 1 MB를 넘는 복사본은 먼저 바이트 비용을 보여 주고, 프로젝트의 증거 총량은 256 MB로 제한됩니다. Repeater 복사본은 탭에 *저장된* 요청(바인딩을 펼치지 않은 상태)과 저장소의 마지막 성공 응답을 짝지으므로 둘이 일치하는 전송 직후에 동결하세요. WebSocket 복사본에는 핸드셰이크만 들어가며 프레임 기록은 포함되지 않습니다. History 상세의 통계 줄과 Repeater의 RESPONSE 테두리에 표시되는 `frozen ×N`은 복사본 개수일 뿐입니다. 원본 탭은 계속 편집하고 전송할 수 있습니다.
...
평소처럼 Repeater 세션들을 Issue에 링크한 뒤, Issue 상세에서 `Space` → **Retest…**(`⇧R`)를 엽니다. `a`로 세션을 고르고 **역할**(`setup`은 전제를 만들고, `baseline`은 본문 비교의 기준, `variant`는 검사 대상, `control`은 대조군, `cleanup`은 되돌리기), 그리고 기대 결과 하나를 정합니다: 상태(`status:403`, `status:2xx`, `status:200-299`), JSON 필드(`json:data.role=admin`, `json-absent:data.token`), 또는 기준과의 비교(`body:same`, `body:diff`). 비워 두면 결과만 기록하고 아무것도 단언하지 않습니다 — 로그인 단계나 cleanup이 원하는 동작입니다. `⇧J` / `⇧K`로 순서를 바꾸고, `e`로 수정, `d`로 삭제, `r`로 실행합니다. `⇧R`은 전송이 거부된 뒤에도 cleanup 단계를 보내도록 허용하며 실행합니다.
