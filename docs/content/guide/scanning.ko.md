...
이미 링크된 이슈에서는 RELATED 카드의 live 행에서 `f`(또는 `Space` → **Manage links** 안에서 `f`)로 그 링크를 동결합니다. 이 규칙 이전에 만든 링크, 출처에 교환이 없던 시점에 걸린 링크, 헤드리스로 추가한 링크가 그 대상입니다. RELATED는 **LIVE** 행과 **FROZEN** 행을 구분해 보여 줍니다. 동결된 행에는 출처(`hist #12`, `repeater #3`)·복사한 시각·상태·크기가 표시되고, `↵`를 누르면 읽기 전용 뷰어가 열립니다(`↹`로 요청/응답 전환, `y`로 보이는 쪽 복사).
