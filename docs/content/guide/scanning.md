...
A link is a pointer, and both things it can point at are mutable: a Repeater tab holds one response and the next send replaces it, and a History flow is one retention sweep from `(gone)`. When the exact bytes matter, **freeze** them. `Space` → **Link & freeze…** (from History, the History detail, or the Repeater) is the same picker restricted to issues: `↵` links the exchange and copies its request, response, status, timing, protocol, error and truncation flags in one transaction, while `+ New issue…` files the issue and freezes in one go. On an issue that is already linked, `f` on a live row of its RELATED card (or `f` in `Space` → **Manage links**) freezes that link. RELATED lists **LIVE** and **FROZEN** rows apart. A frozen row names its source (`hist #12`, `repeater #3`), the time of the copy, its status and size; `↵` opens a read-only viewer (`↹` swaps request/response, `y` copies the shown pane).
