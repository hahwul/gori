...
A link is a pointer, and both things it can point at are mutable: a Repeater tab holds one response and the next send replaces it, and a History flow is one retention sweep from `(gone)`. When the exact bytes matter — the response that confirmed the finding, kept as it was while you retest the fix — **freeze** them. `Space` → **Link & freeze…** (from History, the History detail, or the Repeater) is the same picker restricted to issues: `↵` links the exchange *and* copies its request, response, status, timing, protocol, error and truncation flags into the issue in one transaction, and `+ New issue…` files the issue and freezes in one go. On an issue that is already linked, `f` on a live row of its RELATED card (or `f` in `Space` → **Manage links**) freezes that link. RELATED lists **LIVE** and **FROZEN** rows apart; a frozen row names its source (`hist #12`, `repeater #3`), the moment the copy was taken, its status and its size, and `↵` on it opens a read-only viewer over the detail (`↹` swaps request/response, `y` copies the shown pane) rather than the History drill-in, so nothing there can act on the live flow and `esc` puts you back on the row. An issue keeps as many copies as you take — one before the fix, one after the retest — each with a SHA-256 of its stored request and response, which the Markdown and JSON exports and MCP `get_issue` carry as provenance (never the bytes). A copy is never pruned or replaced; only `Space` → **Delete frozen evidence** (confirmed) or deleting the issue removes it. A Repeater tab that has never been sent is refused rather than frozen request-only, a copy over 1 MB shows its byte cost first, and a project's evidence is bounded at 256 MB. The History detail's stats line and the Repeater's RESPONSE border read `frozen ×N` once a copy of the open flow or tab exists — a marker that a copy exists, not that the flow or tab is locked; the tab stays editable and its next send still replaces the response, which is exactly why the copy was taken.
