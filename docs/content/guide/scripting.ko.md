...
통과**하고 실패는 프로젝트 이벤트 피드에 알림으로 기록됩니다. 멈춘 훅이 치르게 하는 비용은 메시지
하나당 타임아웃 한 번이고, 플로우를 잃는 일은 없습니다. Rewriter에서 타임아웃은 한 메시지의 모든
매치가 나눠 쓰는 *예산*이라, 400번 매치되는 패턴도 타임아웃 400번이 아니라 한 번만큼만 듭니다.
