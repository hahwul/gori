...
Placement가 비용을 결정합니다. `body`는 탭 본문 우하단에 8&times;3 스프라이트를 두며, 세 줄을 가리고 결과를 말풍선으로 알립니다. `bar`는 상태 줄의 CPU/MEM·시계 옆에 일곱 칸짜리 칩을 둡니다. 아무것도 가리지 않고 말풍선도 사라집니다 — 같은 알림의 토스트를 상태 줄이 이미 띄우고 있으니까요. 대신 한 줄로는 얼굴을 담을 공간이 없습니다. Notices는 어느 쪽이든 하단 상태바 토스트와 독립적으로 동작합니다.
