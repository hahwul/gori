...
Face는 마스코트를 하나 더 만드는 설정이 아니라 폰트 호환 설정입니다. `soft`는 쉬는 표정의 입을 `ᴗ`로 그리는데, 이 글리프는 대부분의 고정폭 폰트에 없습니다. 일반적인 데스크톱에서는 터미널이 설치된 다른 폰트에서 같은 한 칸 폭으로 빌려 오므로 모양도 맞고 열도 밀리지 않습니다. 반면 폰트가 거의 없는 최소 환경에는 빌려 올 곳이 없어 네모 박스가 나옵니다. `safe`는 같은 미소를 모든 고정폭 폰트에 있는 `u`로 표기합니다. 박스가 보이면 `safe`로 바꾸면 되고, 그 외에는 아무것도 달라지지 않습니다.
