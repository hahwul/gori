...
**Agent replies** decides how long she keeps saying something an agent sent you with the MCP `reply_to_operator` tool. On `hold`, the default, the reply stays in her bubble (or the status row, in `bar`) until your next key or click, and a job result that lands after it changes her face but not her words; only a newer reply replaces it. A key you were already pressing when it arrived cannot erase it: it still stays for the few seconds any notice gets. `timed` lets a reply go after those few seconds like everything else. Either way it is a notification in the window that is open, not a mailbox: a reply that arrives while no gori TUI is open on the project is not announced later, and the agent is told so.
