...
(`~/.gori`는 gori 홈 디렉터리입니다. `$GORI_HOME`으로 재정의하세요.) 파일 이름이 테마 이름입니다. `ocean.json`은 테마 `ocean`이 됩니다. 커스텀 테마는 선택기에서 내장 테마 다음에 파일 이름 순으로 나타납니다. gori는 시작 시, 그리고 `settings:theme`를 열 때마다 다시 로드하므로, 파일을 넣고 재시작 없이 선택기를 다시 열 수 있습니다.
