...
단위는 밀리초입니다. **타임아웃된** 전송도 `--mt`에서는 매치로 셉니다 — 오리진을 당신이 정한 타임아웃 너머로 밀어낸 페이로드는 같은 신호의 가장 큰 형태인데, 예전에는 그것이 에러로 버려지고 정작 실패한 sleep이 40 ms에 돌아와 보고됐기 때문입니다. 그 밖에는 달라지는 것이 없습니다. 거부되거나 닿지 않은 전송은 여전히 결과가 아니고, `--mt`를 `--mc 200`과 함께 쓰면 응답이 필요한데 타임아웃에는 응답이 없습니다.
