...
스코프는 두 번째 가드레일이며, 액션 도구가 켜져 있어도 유지됩니다. 프로젝트 스코프 밖의, 또는 스코프가 없는 호스트를 겨눈 액티브 도구는 샌드박스가 켜져 있든 아니든 `SCOPE_BLOCKED` 오류로 거부됩니다. 호출에 명시적 예외 선언인 `allow_unscoped:true`를 준 경우만 예외이며, 그때도 샌드박스와 명시적 제외 규칙은 그대로 적용됩니다. 그래서 전체 접근 에이전트조차 스코프를 잡지 않은 호스트로 빗나간 요청을 조용히 보낼 수 없습니다. 이 사전 검사는 MCP에만 있으며, 샌드박스와 명시적 제외 규칙만 적용되는 TUI의 Repeater·Fuzzer보다 엄격합니다.
