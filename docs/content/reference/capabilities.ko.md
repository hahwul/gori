...
| **HTTP/2 WebSocket (RFC 8441)** | 핸드셰이크, h2 프레임, 메시지 트랜스크립트 | 핸드셰이크만 가능. 메시지는 홀드하지 않음 | 미지원. 캡처한 트랜스크립트의 조회·export는 가능 | 미지원 |
...
- **RFC 8441의 메시지 계층은 캡처 전용입니다.** HTTP/2 WebSocket을 인식하고 저장하지만
  Repeater/Fuzzer에서 다시 열거나 메시지를 인터셉트하지는 못합니다. HTTP/1.1 WebSocket은
  세 동작을 모두 지원합니다.
