...
| `--verbatim` | Send the stored bytes exactly: no `$VAR` expansion (project env vars **and** session bindings — a `$NAME` stays literal on the wire), no bare-LF promotion, no `Content-Length` resync, no HTTP/2→1.1 version fix, no h2 field-name lowercasing. The active `--slot`'s header overlay still applies — it answers *as whom*, not *which bytes* |
