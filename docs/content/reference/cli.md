...
```bash
gori run issues create --title "Reflected XSS on /search" --cvss 8.8 --host app.example.com --flow 42 --notes-file finding.md
generate-notes | gori run issues update 7 --status confirmed --notes-stdin --severity critical
...
| `create` | `-t`/`--title` (required), `--cvss` (score or vector; auto-derives severity), `-s`/`--severity` (`info`\|`low`\|`medium`\|`high`\|`critical`), `--host`, `--flow=ID`, `-n`/`--notes`, `--notes-file=PATH`, or `--notes-stdin` |
| `update <id>` | `-t`/`--title`, `--cvss` (new score/vector; empty to clear), `-s`/`--severity`, `-n`/`--notes`, `--notes-file=PATH`, `--notes-stdin`, `--status` (`open`\|`confirmed`\|`false-positive`\|`resolved`) |
...
`--notes`, `--notes-file`, and `--notes-stdin` are mutually exclusive. File and stdin input preserve multiline UTF-8 text and line endings exactly.
