...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `syntax` | string | `"bare"` | 토큰 문법: `namespaced`(환경 변수 `$ENV.KEY`, 세션 바인딩 `$BIND.NAME`) 또는 `bare`(`$KEY`, `$NAME`). **키가 없으면 `bare`**이므로 기존 설치의 문법이 저절로 바뀌지 않습니다. 완전히 새로 만든 gori 홈만 첫 실행에서 `"namespaced"`를 기록합니다. 전환은 [`gori settings env-syntax`](/ko/reference/cli/#env-syntax) — 저장된 토큰은 다시 쓰이지 않습니다 |
