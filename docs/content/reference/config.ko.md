...
이 답은 **주소와 포트**이고, 포트는 결정적입니다. 클라이언트가 실제로 연결한 포트이므로 `target_port`보다도, 클라이언트의 `Host` 헤더에 적힌 포트보다도 우선합니다.

**클라이언트가 보낸 바이트**는 이름을 제공합니다 — 평문은 `Host` 헤더, HTTPS는 핸드셰이크 **이전에** ClientHello에서 읽는 TLS **SNI**. 이름이 있으면 gori는 계속 이름을 씁니다. 이후 모든 결정이 이름을 필요로 하기 때문입니다 — 발급할 leaf 인증서, 샌드박스 게이트, [passthrough 목록](#tls-passthrough), 원 서버 ALPN 프로브, 스코프 매칭, 그리고 History에 보이는 값. 커널의 주소는 이름이 아예 없을 때만 대신 들어갑니다.
...
**조회가 해결하지 못하는 것.** 커널은 주소를 주고 클라이언트는 이름을 주는데, gori는 이름을 우선합니다. 그래서 이름이 있는 목적지는 여전히 DNS로 다시 해석되고, 클라이언트가 실제로 도달한 주소가 아닌 곳으로 갈 수 있습니다. 즉 투명 모드에서 목적지 호스트는 여전히 부분적으로 **클라이언트가 통제**합니다. `Host`로 거짓말하는(또는 오해를 부르는 SNI를 보내는) 클라이언트는 gori의 업스트림 dial 을 다른 호스트로 돌릴 수 있습니다 — 다만 다른 포트로는 더 이상 돌릴 수 없습니다. 배포 형태상 이것이 문제라면 [reverse 모드](#reverse-mode)를 쓰세요. 목적지를 선언하고, 클라이언트가 보낸 것은 아무것도 보지 않습니다.
