...
`off`는 다음 터널 연결부터 적용되며, 원 서버 ALPN 프로브를 아예 생략합니다(원 서버당 연결 1개 절약). 다만 gori가 **정확성을 위해** 수행하는 다운그레이드를 덮어쓰지는 않습니다 — Sandbox, 호스트별 인터셉트, 활성 Match & Replace 규칙은 이 설정과 무관하게 계속 HTTP/1.1을 강제합니다. HTTP/2 릴레이가 실제로 그 seam들을 우회하기 때문입니다. `CONNECT` 안의 평문 HTTP/2(`h2c`) 터널은 `off`일 때 중계하지 않고 거부합니다 — 클라이언트가 preface를 보내며 이미 h2를 확정했으므로 내릴 것이 없습니다.
