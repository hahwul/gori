...
| `agent` | 호스팅된 코딩 에이전트 탭: `command`(argv[0], 기본값 `claude`), `args`(마지막에 덧붙는 추가 argv), `model`(비어 있으면 CLI 자체 기본값), `mcp_read_only`(에이전트 자신의 `gori` MCP 서버를 `--read-only`로 시작, 기본 off), `system_prompt_append`, `permission_policy`(`ask` \| `deny`, `allow`은 없음 — [Settings 가이드](/ko/guide/settings/) 참고), `history_keep`(유지할 대화 턴 수, 기본 50, 1~1000으로 클램프) |
