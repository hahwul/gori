...
| `mcp` | How `gori mcp` delivers "Tell the agent…" messages: `channels` (off by default) enables the `claude/channel` push on top of the inbox-socket, Codex-queue and `operator_messages`-poll layers, which always run. See [Messages from gori](/guide/mcp/#messages-from-gori) |
