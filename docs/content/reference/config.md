...
| `agent` | The hosted coding-agent tab: `command` (argv[0], default `claude`), `args` (extra argv, appended last), `model` (blank = the CLI's own default), `mcp_read_only` (start the agent's own `gori` MCP server `--read-only`; off by default), `system_prompt_append`, `permission_policy` (`ask` \| `deny`, no `allow` — see the [Settings guide](/guide/settings/)) and `history_keep` (conversations kept per project, default 50, clamped 1-1000) |
