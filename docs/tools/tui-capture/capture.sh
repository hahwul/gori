...
OUT="$REPO/docs/static/images/tui"
...
# A minimal settings.json so the first-run wizard is skipped and the default
# theme (goridark) is used. Add "theme":"<name>" to shoot a different palette.
cat > "$GORI_HOME/settings.json" <<'JSON'
{"theme":"goridark","mouse":true,"pretty_bodies":true,
...
echo "▸ done. Review the SVGs in $OUT"
