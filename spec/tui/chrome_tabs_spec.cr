...
describe "Chrome.hidden_tabs" do
  it "returns the tabs hidden from the bar (Rewriter + Miner + Sequencer + JWT + Settings by default) on empty prefs" do
    hid = Chrome.hidden_tabs([] of {String, Bool}).map(&.first)
    hid.should eq([:rewriter, :miner, :sequencer, :jwt, :prefs]) # the default-hidden tabs, in catalog order
  end
