...
#   • the Global fallback, for a letter the tab does not bind at all: a dropped space on a
#     menu `c` silently stops capture.
#
...
module MenuLetterMeaning
  ...
  # scope, then — only for a letter the tab leaves unbound — the Global fallback.
  private def check_keymap(found, keymap : Gori::Verb::Keymap, v : Gori::Verb::Definition, where : String) : Nil
    ...
    if id = keymap.lookup_in(chord, v.scope)
      found[{v.id, id}] << where unless id == v.id
    elsif g = global_fallthrough(keymap, chord, v, e)
      found[{v.id, g}] << where
    end
  end
...
MENU_LETTER_ALLOWED = {
  ...
  {"repeater.pretty-request", "repeater.toggle-pretty"}  => "WP2 #3: bare `p` gets a response-pane gate",
  {"repeater.attach-chain", "repeater.toggle-diff"}      => "WP2 #4: bare `⇧D` gets a response-pane gate",
  ...
  {"fuzzer.clear-selection", "fuzz.dist"}                => "WP2 #13: bare `v` gets a results-pane gate",
