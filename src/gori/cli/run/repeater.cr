    ...
    module Run
      ...
      private def self.cmd_repeater_send(args : Array(String)) : Nil
        ...
        parser = OptionParser.new do |p|
          ...
          p.on("--allow-unscoped", "Send even if the target is outside the project scope (Sandbox/exclude still apply)") { allow_unscoped = true }
          p.on("--verbatim", "Send the stored bytes EXACTLY: no $VAR expansion (project env vars AND session bindings — a $NAME stays literal on the wire), no bare-LF→CRLF promotion, no Content-Length resync, no HTTP/2→1.1 version fix, and on h2 no field-name lowercasing. The active --slot's header overlay still applies: it answers a different question (send this AS WHOM) — select `as-captured`, or no slot, to send the stored headers") { verbatim = true }
