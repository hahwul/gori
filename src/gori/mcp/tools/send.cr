    ...
    class Tools
      ...
      private def list_send_tools(j : JSON::Builder) : Nil
        ...
        tool j, "send_websocket",
          ...
          s.field "verbatim", boolprop("send the bytes EXACTLY: no env expansion — project env vars AND session bindings — in the handshake head or in any frame payload, no bare-LF→CRLF promotion, no Content-Length resync. The active session slot's header overlay still applies to the handshake. Use it when a literal $ENV.KEY / $BIND.NAME token (bare syntax: $KEY / $NAME) is the payload (default false)")
        end
      end
    end
  end
