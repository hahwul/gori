...
module Gori::Tui
  ...
  class ConfirmDialog
    ...
    private def render_buttons(screen : Screen, box : Rect) : Nil
      ...
      y = box.bottom - 3
      x = render_button(screen, x, y, @confirm_label, @selected == :confirm, @danger)
      render_button(screen, x + 4, y, @cancel_label, @selected == :cancel, false)
    end
