...
module Gori::Tui
  ...
  class CookieView
    ...
    def render_decode(screen : Screen, rect : Rect, *, input : TextArea, input_mode : InputMode,
                      ...
                      algorithm : String, salt : String, salt_cx : Int32, salt_pre : String,
                      verify_state : Symbol, crack_note : String?, secret : String, secret_cx : Int32,
      ...
      render_opts(screen, opts_c, format, algorithm, salt, salt_cx, salt_pre, focused && pane == :opts) unless opts_c.empty?
    ...
    def render_forge(screen : Screen, rect : Rect, *, payload : TextArea, format : String,
                     algorithm : String, salt : String, salt_cx : Int32, salt_pre : String,
                     secret : String, secret_cx : Int32, secret_pre : String, output : String,
                     output_ok : Bool, pane : Symbol, focused : Bool, lens_chord : String) : Nil
      return if rect.empty?
    ...
    private def render_opts(screen : Screen, card : Rect, format : String, algorithm : String,
                            salt : String, cx : Int32, pre : String, active : Bool) : Nil
      ...
      # algo:<hmac> — a click-only chip chained left of it, drawn only for Django (else inert).
      Frame.toggle_badge(screen, fx, card.y, min_x, "algo", algorithm, true) if format == "django"
      c = card.inset(1, 1)
      return if c.h <= 0
      ...
      if format == "rack"
        ...
        screen.text(c.x + 5, c.y, salt_placeholder(format), Theme.muted, Theme.bg, width: vw)
      else
    ...
    # knows what is in effect without typing it (a Django SESSION cookie needs SESSION_SALT).
    private def salt_placeholder(format : String) : String
      case format
      when "flask"  then "(default: #{Cookie::Flask::SALT})"
      when "django" then "(default: #{Cookie::Django::DEFAULT_SALT})"
      else               "(default salt)"
      end
    end

    # Hit-test the OPTIONS badges. Returns :format, :algorithm, or nil. `format` decides whether
    # the algorithm badge was drawn at all (Django only), so the chain is built to match render.
    def opts_badge_hit(card : Rect, mx : Int32, my : Int32, format : String, algorithm : String) : Symbol?
      badges = [{:format, "^A", format}] of {Symbol, String, String}
      badges << {:algorithm, "algo", algorithm} if format == "django"
