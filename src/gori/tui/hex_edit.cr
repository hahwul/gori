  ...
  class HexEdit
    ...
    def render(screen : Screen, rect : Rect, focused : Bool, scroll : Int32) : Int32
      ...
      (0...rect.h).each do |i|
        row = scroll + i
        break if row >= total
        draw_row(screen, rect.x, rect.y + i, row, focused)
      end
      scroll
    end

    private def draw_row(screen : Screen, x : Int32, y : Int32, row : Int32, focused : Bool) : Nil
      off = row * COLS
      cur_b = @nib // 2
      cur_hi = @nib.even?
      screen.text(x, y, "%08x" % off, Theme::MUTED)
      hx = x + 10
      ...
      # ASCII gutter: |....|, cursor byte inverted
      ax = hx + 1
      screen.cell(ax, y, '|', Theme::MUTED)
      (0...COLS).each do |col|
        idx = off + col
        break if idx >= @bytes.size
        b = @bytes[idx]
        ch = (b >= 0x20_u8 && b <= 0x7e_u8) ? b.unsafe_chr : '.'
        cur = focused && idx == cur_b
        screen.cell(ax + 1 + col, y, ch, cur ? Theme::BG : Theme::MUTED, cur ? Theme::ACCENT : Theme::BG)
      end
      screen.cell(ax + 1 + {len - off, COLS}.min.clamp(0, COLS), y, '|', Theme::MUTED) if off < len
